...
def _parse_lines(content: bytes) -> list[dict[str, Any]]:
    ...
    if len(content) > MAX_UPLOAD_BYTES:
        raise InvalidImport(
            f"Import file exceeds the {MAX_UPLOAD_BYTES}-byte upload limit"
        )
    records: list[dict[str, Any]] = []
    for line_number, encoded_line in enumerate(io.BytesIO(content), start=1):
        try:
...
def _events_to_traces(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    observation_traces: dict[str, set[str]] = defaultdict(set)
    for event in records:
        ...
        if observation_id and not trace_id:
            matches = observation_traces.get(observation_id, set())
            if len(matches) == 1:
        ...
        observation_traces[observation_id].add(trace_id)
...
class LangfuseJSONLImporter:
    ...
    def _normalize_session(
        ...
        for trace_id, observations in traces:
            ...
            for record in ordered:
                observation_id = str(
                    record.get("id") or (trace_id if len(ordered) == 1 else "")
                )
...
def parse(
    ...
    context = ImportContext.from_params(params)
