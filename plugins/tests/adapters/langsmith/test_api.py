...
async def test_fetch_by_trace_ids_fetches_exactly_those_in_order(
    ...
    payloads = await _collect({"trace_ids": [trace_id_1, trace_id_2]})
...
async def test_fetch_time_window_falls_back_to_the_default_project(
    ...
    await _collect({"since": "2026-07-01T00:00:00Z"})
...
async def test_fetch_yields_nothing_for_an_empty_listing(
    ...
    payloads = await _collect({"since": "2026-07-01T00:00:00Z"})
...
async def test_fetch_requires_since_without_trace_ids(
    fake_langsmith_api: FakeLangSmith,
) -> None:
    """Require since when trace_ids is absent."""
    with pytest.raises(ValueError, match="since is required when trace_ids is absent"):
        await anext(fetch({}))


async def test_fetch_rejects_non_list_trace_ids(
    fake_langsmith_api: FakeLangSmith,
) -> None:
    """Reject a trace_ids value that is not a list."""
    with pytest.raises(ValueError, match="Input should be a valid list"):
        await anext(fetch({"trace_ids": "not-a-list"}))


async def test_fetch_rejects_an_unparsable_since(
    fake_langsmith_api: FakeLangSmith,
) -> None:
    """Reject a since value that is not a valid ISO 8601 datetime."""
    with pytest.raises(ValueError, match="Input should be a valid datetime"):
        await anext(fetch({"since": "not-a-date"}))


async def test_fetch_rejects_an_unparsable_until(
    fake_langsmith_api: FakeLangSmith,
) -> None:
    """Reject an until value that is not a valid ISO 8601 datetime."""
    with pytest.raises(ValueError, match="Input should be a valid datetime"):
        await anext(fetch({"since": "2026-07-01T00:00:00Z", "until": "not-a-date"}))


async def test_fetch_rejects_a_naive_since(
    fake_langsmith_api: FakeLangSmith,
) -> None:
    """Reject a since value without timezone information."""
    with pytest.raises(ValueError, match="Input should have timezone info"):
        await anext(fetch({"since": "2026-07-01T00:00:00"}))


async def test_fetch_rejects_until_before_since(
    fake_langsmith_api: FakeLangSmith,
) -> None:
    """Reject a query whose until precedes since."""
