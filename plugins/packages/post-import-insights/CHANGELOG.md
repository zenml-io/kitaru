...
- Register separate deterministic and OpenAI analyzers so callers can select either or both for an import. The OpenAI analyzer defaults to `gpt-5.6-luna`; callers can override its `model` parameter and must provide OpenAI credentials.
