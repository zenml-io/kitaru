...
Spec for `kitaru/job/`, the code that runs inside a job process. The worker (see `worker.md`) spawns these processes: `python -m kitaru.job evaluate` for evaluation jobs, `python -m kitaru.job import` for import jobs, and the agent's own command for replays and session runs, whose code imports the accessors at the package root. The server and the async API client (`KitaruAPIClient`) exist and are taken as given. This document contains everything needed to implement the package.
...
| `KITARU_JOB_PLUGIN_PATH` | `evaluator`, `importer` | Path of the materialized script plugin file, unset for package plugins |
| `KITARU_JOB_PAYLOAD_PATH` | `importer` | Path of the materialized payload file |
| `KITARU_JOB_RESULT_PATH` | `evaluator`, `importer` | Path the result JSON is written to |

## API surface used

| Call | Used by | Purpose |
|---|---|---|
| `client.jobs.get_spec(job_id)` | `evaluator`, `importer`, root accessor fallback | Resolve the job spec |
...
src/kitaru/job/
  ...
  __main__.py      # process entry: python -m kitaru.job <evaluate|import>
...
3. Open a `KitaruAPIClient` via `KitaruAPIClient.from_env()` (which requires `KITARU_API_URL` and reads `KITARU_API_KEY`) and `asyncio.run` the selected flow: `evaluator.run(client, job_id)` or `importer.run(client, job_id)`.
...
1. Fetch the spec, require evaluation details.
2. Resolve the evaluator callable: a script plugin via `load_plugin_entrypoint(KITARU_JOB_PLUGIN_PATH, details.plugin.entrypoint, ...)`, a package plugin via `load_source_ref(details.plugin.entrypoint, ...)`.
...
5. Write the `list[EvaluationResult]` via `write_job_result`.
...
- **Evaluator**: a callable `def evaluate(session: SessionView, **params) -> EvaluationResult | list[EvaluationResult]`, each result becomes one evaluation row. `SessionView` and `EvaluationResult` are imported from `kitaru.job.evaluator`.
