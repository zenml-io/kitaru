...
<Callout type="warn">
  **Apple Silicon (M-series) Macs:** The published image is currently amd64-only.
  Docker Desktop can run it under Rosetta emulation, but you must add the
  `--platform` flag:
  ...
  Without `--platform linux/amd64`, `docker pull` and `docker run` will fail
...
- If you see `ConnectionRefused` on port **8383**, you likely ran bare
  `kitaru login` without a URL. That command starts a local server process —
  it does not connect to a Docker container. Use
  `kitaru login http://localhost:8080` to connect to your Docker server.
...
- [Stacks](/stacks)
- [Kubernetes Stacks](/stacks/kubernetes-stacks)
