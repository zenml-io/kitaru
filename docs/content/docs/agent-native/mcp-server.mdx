...
| Tool | Use it for |
|---|---|
| `deploy_flow` | Create a new deployment version from `<module_or_file>:<flow_name>` |
| `invoke_flow` | Start a new execution from a deployed flow by `default`, tag, or version |
| `list_flow_deployments` | List all deployment versions, optionally filtered to one flow |
| `get_flow_deployment` | Inspect one deployment by version or tag |
| `delete_flow_deployment` | Delete one version when no exclusive tag protects it |
| `tag_flow_deployment` | Attach or move a public tag to a version |
| `untag_flow_deployment` | Remove a non-reserved public tag from a version |

`deploy_flow` accepts deployment-time flow inputs plus optional deployment
controls:
...
`invoke_flow` is the MCP equivalent of the primary CLI command
...
Deployment MCP calls do not use per-deployment tokens. `deploy_flow`,
`invoke_flow`, and the deployment management tools authorize through the active
