...
- `kitaru_memory_compact` — summarize memory values with an LLM and write
  the result. Use `key` for single-key history summarization, or `keys` (a
  list) with `target_key` for multi-key merging. Source entries are not deleted.
...
1. `kitaru_memory_compact(scope="repo_docs", keys=["conventions/test_runner", "conventions/python"], target_key="summaries/conventions")`
2. `kitaru_memory_purge(key="conventions/test_runner", scope="repo_docs", keep=1)`
3. `kitaru_memory_compaction_log(scope="repo_docs")`
