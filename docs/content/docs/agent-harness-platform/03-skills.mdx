...
<Callout type="info">
  This walkthrough uses `stage_3_skills.py` from the runnable Agent Harness Platform example. If you have not cloned the repo yet, start with [Get the code](/agent-harness-platform/#get-the-code) on the overview page.
</Callout>
...
Stage 3 runs its shell commands in the same Docker sandbox as [Stage 2](/agent-harness-platform/02-sandbox). If you skipped that stage, build the image once:
