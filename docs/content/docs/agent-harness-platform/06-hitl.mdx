...
[Stage 5](/agent-harness-platform/05-typed-services) narrowed what the agent can do. It reasons with `exec` inside the sandbox and takes specific actions with `exec_service`, and the profile decides which actions are even on the menu. That is a tight boundary. It still is not judgment.
...
<Callout type="info">
  This walkthrough uses `stage_6_hitl.py` from the runnable Agent Harness Platform example. If you have not cloned the repo yet, start with [Get the code](/agent-harness-platform/#get-the-code) on the overview page.
</Callout>
...
1. Look up `lookup_wiki(topic="durability")`, the host-side typed service from [Stage 5](/agent-harness-platform/05-typed-services).
...
If you drop the teaching-only `granular_checkpoints=False` and run granular (the upgrade [Stage 1](/agent-harness-platform/01-durable-agent) describes), exempt `ask_question` from per-tool checkpoints by passing `tool_checkpoint_config_by_name={"ask_question": False}` to `KitaruAgent`. The other tools (`exec`, `skill`, `exec_service`) stay granular.
...
Replay is not a stage of its own here. It is a general Kitaru primitive that works the same on any flow, not something specific to agent harness platforms, so it lives in [Replay and Overrides](/guides/replay-and-overrides) rather than on its own Agent Harness Platform page.

If you are weighing this pattern for real use, [Production notes and upgrade paths](/agent-harness-platform/production-notes) gathers every teaching stand-in from the tour in one place and lists what to harden before you depend on it.
