...
The durability story is identical either way. The only change is how finely work is cached and how finely it shows up on the dashboard. Nothing about Kitaru itself changes here. See [Stage 1](/agent-harness-platform/01-durable-agent).
...
The agent's tool wiring does not change when you swap, because the swap happens behind `run(command)`. See [Stage 2](/agent-harness-platform/02-sandbox).
...
The seam is `SandboxProxyRule` plus the proxy container. See [Stage 4](/agent-harness-platform/04-credential-proxy).
...
Adding a service is three files: a handler, an args and result Pydantic pair, and an `ALL_SERVICES` entry. The tool surface and its description update from the profile's `allowed_services`. See [Stage 5](/agent-harness-platform/05-typed-services).
...
See [Stage 6](/agent-harness-platform/06-hitl).
