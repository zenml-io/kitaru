...
For restart durability, use a persistent LangGraph checkpointer such as the ones documented in the [LangGraph persistence guide](https://docs.langchain.com/oss/python/langgraph/persistence). Kitaru records the outer execution and preserves the `thread_id`; LangGraph's checkpointer is still the thing that stores graph-internal state.
...
|---|---:|---|
| `save_input` | `True` | Include graph-call input in captured adapter metadata/artifacts where applicable. |
| `save_output` | `True` | Include completed graph output. Interrupted runs do not store raw interrupt SDK objects as output. |
...
| `save_state_snapshot` | `True` | Inspect `graph.get_state(config)` when available and save a safe summary. |
| `save_state_values` | `False` | Do not save full LangGraph state values unless you opt in. |
| `save_state_tasks` | `True` | Save task metadata summaries so interrupts are easier to debug. |

If your graph state contains prompts, tool outputs, customer data, or secrets, be careful with `save_state_values=True`.
...
- **Restart durability does not work with `InMemorySaver`** — use a persistent LangGraph checkpointer. `InMemorySaver` is only in-memory.
