...
When the adapter persists from checkpoint scope, it saves two Kitaru context artifacts for each graph call:

- `event_log__<graph>_<run_label>` — the ordered LangGraph adapter events, such as `graph_call_started`, `graph_call_completed`, `graph_interrupted`, or `graph_call_failed`.
- `run_summary__<graph>_<run_label>` — the run summary: thread ID, status, captured config/context fields, output or failure details, warnings, and observed LangGraph checkpoint metadata.

Inside checkpoint scope, Kitaru logs lightweight metadata pointers to those artifacts for search and debugging. If you call the runner outside checkpoint scope, Kitaru cannot call `kitaru.save(...)`; in that case the adapter logs the full event/run-summary payload as scope-appropriate metadata instead.
...
- Save event logs and run summaries as role-first Kitaru context artifacts when the graph call is captured inside checkpoint scope.
...
The adapter is named `langgraph` because LangGraph is the runtime seam. LangChain agents and Deep Agents are built on top of LangGraph-style execution, but they add their own higher-level concepts. This release only ships the `langgraph` extra; if your graph depends on LangChain or Deep Agents packages, install those packages directly in your project.
...
| LangChain `create_agent(...)` | Not a separate Kitaru extra. You can try wrapping the returned object only if it behaves like a compatible LangGraph runnable with `invoke` / `ainvoke` / `get_state`. |
| Deep Agents | Not a separate Kitaru extra. Invocation can be wrapped if compatible, but Deep Agents filesystem, sandbox, and backend semantics remain Deep Agents-owned. |
...
5. The flow saves a `summary__langgraph_demo` artifact.
