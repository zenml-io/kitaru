...
Kitaru's LangGraph adapter lets you run an existing [LangGraph](https://docs.langchain.com/oss/python/langgraph/overview) graph from a Kitaru flow without taking LangGraph's state machinery away from LangGraph.

There are two checkpoint strategies:

- **`graph_call`** — the default. Kitaru puts one durable checkpoint around the outer `graph.invoke(...)` or `graph.ainvoke(...)` call. This works for any compatible LangGraph graph.
- **`calls`** — opt-in. Kitaru does **not** create the outer graph-call checkpoint. Instead, synchronous LangChain model/tool calls can become their own Kitaru checkpoints when the graph was built with `KitaruLangGraphMiddleware`.

The important boundary is concrete:
...
So Kitaru is not replacing LangGraph persistence. It is adding Kitaru durability and observability at the places where Kitaru can safely stand.
...
The `langgraph` extra includes the LangChain package used by calls mode.
...
Think of `graph_call` as putting one Kitaru shipping label on the whole LangGraph box.
...
`calls` mode is different. Kitaru still does not split open arbitrary LangGraph nodes. Instead, you put Kitaru middleware at the LangChain model/tool call seam:
...
That second picture is the key. Kitaru is not magically seeing through LangGraph. The middleware is physically around the real LangChain handler call, so Kitaru can create a true checkpoint there.

This avoids a dangerous double-replay problem. Imagine a graph node sends a Slack message, the process crashes, LangGraph resumes from its last checkpoint, and Kitaru also retries the same node. The message might be sent twice. The default `graph_call` strategy avoids that by using one Kitaru boundary around the graph call. The `calls` strategy is narrower: it only checkpoints calls where Kitaru middleware is actually wrapped around the model/tool handler.
...
The default call checkpoint types are `model_call`, `tool_call`, and `langgraph_summary`. Adapter-created call checkpoints run inline and default to no cache/no retries.
...
| `save_model_input` | `True` | In sync calls mode, store model input as a structural checkpoint input when a true model checkpoint is opened. |
| `save_model_response` | `True` | In sync calls mode, record the model checkpoint output as the canonical response artifact reference. |
| `save_model_usage` | `True` | Include model usage metadata when the response exposes it. |
| `save_tool_args` | `True` | In sync calls mode, store tool arguments as a structural checkpoint input when a true tool checkpoint is opened. |
| `save_tool_result` | `True` | In sync calls mode, record the tool checkpoint output as the canonical result artifact reference. |
...
- In `calls` mode, model/tool checkpoints are separate, and the aggregate event/run artifacts are saved from a `langgraph_summary__<graph>_<run_label>` checkpoint when possible.
...
The included example is intentionally deterministic. It uses no provider API key.
...
1. Builds a deterministic LangChain agent with a fake local chat model.
2. The fake model requests one `approve_ticket` tool call.
3. `KitaruLangGraphMiddleware` creates sync call checkpoints around the model/tool handlers.
4. Kitaru records `model_call__...`, `tool_call__approve_ticket_...`, and `langgraph_summary__...` checkpoints.
5. The flow saves the same user-facing `summary__langgraph_demo` artifact.
...
LangGraph adapter local demo summary (graph_call):
- strategy: graph_call
...
Kitaru: Checkpoint `tool_call__approve_ticket_...` started.
Kitaru: Checkpoint `langgraph_summary__...` started.
LangGraph adapter local demo summary (calls):
- strategy: calls
- final_message: Ticket ticket-42 approved by fake local model.
```
...
- **"requires LangChain"** — calls mode middleware needs LangChain. Install the `langgraph` extra, which includes it.
