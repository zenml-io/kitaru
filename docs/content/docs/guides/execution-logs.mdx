...
When you start a flow from Python and then wait for it in an interactive
terminal, Kitaru follows live backend step log lines by default:

```python
handle = my_flow.run(topic="renewable energy")
result = handle.wait()  # follows live checkpoint logs while waiting
```

This uses the same follow loop as `kitaru executions logs --follow`, including
polling the backend, deduplicating already-seen log entries, and stopping when
the execution reaches a terminal state. If runtime log retrieval is not
available for the current backend, `wait()` falls back to the older silent
status-polling behavior and still returns or raises from the final execution
status.

By default, `wait()` auto-prints log lines only when stdout is interactive
(for example, in a terminal). If stdout is non-interactive and you do not
provide `log_sink`, `wait()` does **not** start the log-follow loop; it uses
normal status polling instead so CI jobs, notebooks, and library code do not
create hidden backend log-retrieval traffic.

To keep the previous silent behavior explicitly, opt out:

```python
result = handle.wait(stream_logs=False)
```

You can also receive raw log entries in your own callback instead of printing
plain messages to stdout. This is the opt-in path for streaming logs in CI,
notebooks, services, and other non-interactive contexts:

```python
def collect(entry):
    print(entry.timestamp, entry.checkpoint_name, entry.message)

result = handle.wait(log_sink=collect)
```

`wait()` log following begins only after `.run()` returns a `FlowHandle`.
If submission itself is still in progress, that pre-handle phase remains silent
in this follow-up.

## Python SDK log retrieval example
