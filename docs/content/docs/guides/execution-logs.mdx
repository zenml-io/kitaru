...
When you start a flow from Python and then wait for it, Kitaru follows live
backend step log lines by default:
...
(for example, in a terminal). In non-interactive/library contexts, `wait()`
still follows logs but does not print unless you provide `log_sink=...`.
