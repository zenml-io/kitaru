...
- `KITARU_LLM_MOCK_RESPONSE` produces a text-only mocked `LLMResponse` for
  tests and demos.
- `KITARU_LLM_MOCK_RESPONSE_JSON` produces a structured mocked `LLMResponse`,
  including tool calls, for richer fixture tests and examples.
