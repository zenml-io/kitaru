...
```bash
uv sync --extra local --extra pydantic-ai
uv run examples/pydantic_ai_agent/pydantic_ai_adapter.py       # base wrap
uv run examples/pydantic_ai_agent/pydantic_ai_hitl_input.py    # structured HITL input
uv run pytest tests/test_phase17_pydantic_ai_example.py
```

The adapter example prints:
...
The HITL-input example pauses for a `BugReport` from the human (terminal prompt locally, or `kitaru executions input` remotely).
