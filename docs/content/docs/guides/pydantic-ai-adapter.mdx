...
A wait inside a checkpoint pauses the flow; on resume, the enclosing checkpoint re-runs from the top. That's cheap for tight checkpoints (one agent turn, one wait), but it means expensive work placed before the wait is repeated. The default granular mode splits each top-level model, tool, and MCP call into its own checkpoint, which improves visibility and retry isolation; see [Checkpoint modes](#checkpoint-modes) below for the current replay-cost caveat.
