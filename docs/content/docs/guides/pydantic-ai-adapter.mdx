...
<Callout type="info">
  **Compatibility.** Existing code that passes `granular_checkpoints=True` or
  `granular_checkpoints=False` still works. The preferred spelling is now
  `checkpoint_strategy="calls"` for the old `True` behavior and
  `checkpoint_strategy="turn"` for the old `False` behavior.
</Callout>
