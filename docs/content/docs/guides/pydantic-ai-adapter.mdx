...
### Free-form structured input

`schema` accepts any Python type — including a Pydantic model — not just `bool`. The validated value returned from `kitaru.wait(...)` becomes the tool's return value, so the agent continues as if the tool produced it.
...
def collect_bug_report() -> BugReport:
    # Body is skipped in HITL mode; schema drives the wait.
...
See [`examples/pydantic_ai_agent/pydantic_ai_hitl_input.py`](https://github.com/zenml-io/kitaru/blob/develop/examples/pydantic_ai_agent/pydantic_ai_hitl_input.py) for the full runnable example.
