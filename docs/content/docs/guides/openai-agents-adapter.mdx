...
Streaming is not transcript durability. Live token chunks are like radio chatter
during the run: useful to watch, but not the replay source of truth yet. The
durable value is the completed checkpoint result. And because a cached result
skips the live model call, a streaming demo run with caching on may emit no live
events at all — good for durability, distracting for a screen recording — so use
`@flow(cache=False)` for demos where the point is to watch fresh events.
...
To run the same example with live stream events, set `OPENAI_AGENTS_STREAM=1`:
