...
5. saves the request envelope and normalized `LLMResponse` artifacts, then logs usage and latency metadata
...
- **Read the secret inside a flow or checkpoint.** Call `kitaru.get_secret()`
  where the credential is needed. See [Manage Secrets](/guides/secrets) for an
  example.
- **Expose the secret's keys as environment variables for the whole run.**
  ...
  paths. See
  [Containerization → Secret-backed environment variables](/guides/containerization#secret-backed-environment-variables)
  for the full story.
