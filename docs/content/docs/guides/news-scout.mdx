---
title: News Scout
description: An agentic news monitor with granular per-tool checkpoints, explicit run inputs, and remote deploy via image config
---

The `news_scout` example (under `examples/end_to_end/news_scout/`) is a runnable PydanticAI agent that searches news sources, investigates articles, and scores what's worth surfacing. Every model request and every tool call is its own Kitaru checkpoint — individually cached, individually replayable, and visible in the dashboard.
...
- Explicit run inputs via `--interests`, passed into the flow as concrete Python values
...
cd kitaru/examples/end_to_end/news_scout
```
...
Run once with the built-in interest list, or pass a custom list for that run:
...
That constraint has a practical consequence: the agent should receive ordinary Python values, not unresolved runtime handles. The example keeps the CLI parsing outside the flow, chooses either `--interests` or the built-in defaults, and passes the resulting list directly into `news_scout.run(...)`:
...
<Callout type="info">
  If you want a saved profile, store it in your own database, config file, or
  service, then load it before calling the flow. Kitaru keeps the execution
  durable once the concrete values are passed in.
</Callout>
