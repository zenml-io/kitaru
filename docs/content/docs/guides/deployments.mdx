...
One important practical detail: external HTTP invocation only works for
deployments that the Kitaru server can run by itself. If you deployed a version
on a local/default stack, Kitaru can still list it as a deployment version, but
the remote server may reject a curl invocation because that version depends on
local machine components. For HTTP-triggered deployments, deploy on a remote
stack such as Kubernetes, Vertex, SageMaker, or AzureML.
