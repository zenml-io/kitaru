...
JSON object matching `kitaru.ImageSettings`, or `@file`. That image config is
saved into the deployment snapshot. Later invokes can override flow inputs, but
they do not rewrite the deployment image.
