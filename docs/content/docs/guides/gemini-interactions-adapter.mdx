...
- safe step summaries from `interaction.steps` so your flow can respond to
  handoff points without storing raw provider payloads by default
...
preview-shaped than a simple model interaction.
...
- event log
- run summary
- safe step summaries on the returned result
