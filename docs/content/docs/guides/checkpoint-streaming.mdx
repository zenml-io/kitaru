...
description: Publish best-effort live progress and custom events from running checkpoints
---
...
Live-event publishing uses the active Kitaru backend's streaming support. If the
backend has streaming enabled, events can be consumed by backend-side tooling or
future Kitaru watcher APIs.

If streaming is unavailable, publishing degrades safely: the checkpoint keeps
running, and event delivery is skipped. That is deliberate. A broken live-event
lane should not break your training job, tool call, or agent step.

Kitaru does not yet ship a public Python watcher or CLI watcher in this PR. This
page covers publishing only.
