...
`pydantic_ai.stream.started`, `pydantic_ai.stream.event`,
`pydantic_ai.stream.completed`, and `pydantic_ai.stream.failed` when you use
PydanticAI streaming hooks. The mechanism is the same as custom checkpoint live
events: watch them with `KitaruClient().executions.events(...)`, and treat them
as best-effort progress rather than saved state.

See [PydanticAI Adapter](/guides/pydantic-ai-adapter/#streaming) for the
recommended `event_stream_handler` pattern and a runnable streaming example.
