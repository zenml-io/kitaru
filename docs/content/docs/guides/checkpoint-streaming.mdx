...
Checkpoint filtering is guaranteed client-side. Kitaru also sends known step
names to the backend when they are already visible, which can reduce traffic,
but a live execution may create steps after the watcher starts.
