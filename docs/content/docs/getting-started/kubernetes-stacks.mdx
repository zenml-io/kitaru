...
Use this page for the story and the happy path. For exact flag syntax and every supported option, use the generated CLI reference for [`kitaru stack create`](/cli/stack/create), [`kitaru stack show`](/cli/stack/show), [`kitaru stack delete`](/cli/stack/delete), and [`kitaru run`](/cli/run).
...
```bash
kitaru run path/to/flow.py:my_flow --stack prod-k8s
```
...
- `kitaru run ... --stack ...` changes only that one execution
