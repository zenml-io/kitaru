...
description: Store provider credentials, register a local model alias, and use kitaru.llm() inside a flow
---

This walkthrough shows the full setup path for tracked LLM calls in Kitaru:

1. store provider credentials in a secret
2. register a local model alias that points at that secret
...
## 2) Register a local model alias
...
<Callout type="info">
  ...
  itself. The first alias you register also becomes your default local alias
  automatically.
</Callout>
...
2. checks whether that value is a local alias
...
<Callout type="info">
  ...
  first, then `KITARU_DEFAULT_MODEL`, then your local default alias. If
...
[Manage Secrets](/getting-started/secrets) for the current advanced low-level
pattern.
