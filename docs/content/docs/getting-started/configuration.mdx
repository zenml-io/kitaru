...
1. Invocation-time overrides (`my_flow.run(..., runner="prod")`)
2. Flow decorator defaults (`@flow(runner="prod")`)
3. `kitaru.configure(...)`
4. Environment variables (`KITARU_RUNNER`, `KITARU_SERVER_URL`, etc.)
5. Project config (`pyproject.toml` under `[tool.kitaru]`)
6. Global user config (including active runner and connection defaults)
7. Built-in defaults (`cache=True`, `retries=0`)

In practice, `runner` starts unset in the built-in defaults and is then resolved
from the active runner when one is available.
...
```bash
uv sync --extra local
uv run -m examples.basic_flow.flow_with_configuration
...
- [Runner Selection](/getting-started/runner-selection)
