...
| `dapr` | Experimental Dapr workflow backend (`dapr run`, Dapr state store, durable workflow worker) |

```bash
uv add "kitaru[mcp,pydantic-ai,local,dapr]"
# or: pip install "kitaru[mcp,pydantic-ai,local,dapr]"
```
