...
Today, the CLI and MCP server can provision three shipped stack types:
...
One important scope note: the public Python SDK `kitaru.create_stack(...)` currently provisions local stacks only. Kubernetes and Vertex stack creation are exposed through the CLI and MCP surfaces.
