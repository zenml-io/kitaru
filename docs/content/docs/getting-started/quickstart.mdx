...
That&rsquo;s the runtime layer running on your laptop. The same flow survives
...
platform (auth, policy, UI) keeps sitting in front. When you&rsquo;re ready,
`my_agent.deploy(...)` captures an immutable versioned snapshot that your
platform invokes by flow name &mdash; no per-deployment tokens, no separate
service per version.
...
<Cards>
  <Card
    title="Agent Harness Platform"
    description="The recommended end-to-end tour: build a durable agent harness platform stage by stage on Kitaru + PydanticAI"
    href="/agent-harness-platform"
  />
  <Card
    title="Core Concepts"
