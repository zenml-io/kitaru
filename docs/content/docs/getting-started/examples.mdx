...
| `integrations/pydantic_ai_agent/pydantic_ai_streaming.py` | Watch best-effort `pydantic_ai.stream.*` live events while `.wait()` returns the durable final answer | [PydanticAI Adapter](/guides/pydantic-ai-adapter#streaming) |
...
| `integrations/openai_agents_agent/openai_agents_streaming.py` | Watch best-effort `openai_agents.stream.*` live events while `.wait()` returns the durable `OpenAIRunResult` | [OpenAI Agents Adapter](/guides/openai-agents-adapter#streaming-with-kitaru-durability) |
...
| `integrations/claude_agent_sdk_agent/claude_agent_sdk_streaming.py` | Watch best-effort `claude_agent_sdk.stream.*` live events while `.wait()` returns the durable `ClaudeRunResult` (`ANTHROPIC_API_KEY` or Claude SDK provider credentials required) | [Claude Agent SDK Adapter](/guides/claude-agent-sdk-adapter#live-streaming-with-kitaru-durability) |
...
| `integrations/langgraph_agent/langgraph_streaming.py` | Watch best-effort `langgraph.stream.*` live events from a local graph-call stream while `.wait()` returns the durable `LangGraphRunResult` | [LangGraph Adapter](/guides/langgraph-adapter#graph-call-streaming) |
...
10. `integrations/pydantic_ai_agent/pydantic_ai_streaming.py` — [PydanticAI streaming](/guides/pydantic-ai-adapter#streaming).
11. `integrations/openai_agents_agent/openai_agents_adapter.py` — [OpenAI Agents Adapter](/guides/openai-agents-adapter).
12. `integrations/openai_agents_agent/openai_agents_streaming.py` — [OpenAI Agents streaming](/guides/openai-agents-adapter#streaming-with-kitaru-durability).
13. `integrations/claude_agent_sdk_agent/claude_agent_sdk_adapter.py` — [Claude Agent SDK Adapter](/guides/claude-agent-sdk-adapter).
14. `integrations/claude_agent_sdk_agent/claude_agent_sdk_streaming.py` — [Claude Agent SDK streaming](/guides/claude-agent-sdk-adapter#live-streaming-with-kitaru-durability).
15. `integrations/langgraph_agent/langgraph_adapter.py` — [LangGraph Adapter](/guides/langgraph-adapter).
16. `integrations/langgraph_agent/langgraph_streaming.py` — [LangGraph streaming](/guides/langgraph-adapter#graph-call-streaming).
17. `end_to_end/openai_research_bot/research_bot.py` — [Research bot](/guides/openai-agents-adapter#end-to-end-research-bot-example).
18. `features/mcp/mcp_query_tools.py` — [MCP Server](/agent-native/mcp-server).
19. **[Agent Harness Platform](/agent-harness-platform)** — the same primitives, woven into one runnable agent harness platform.
