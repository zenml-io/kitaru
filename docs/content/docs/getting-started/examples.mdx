...
| Compare LangGraph graph-call vs calls-mode checkpoints | `integrations/langgraph_agent/` | `uv run examples/integrations/langgraph_agent/langgraph_adapter.py --strategy graph_call` or `--strategy calls` |
...
| `integrations/langgraph_agent/langgraph_adapter.py` | Local deterministic LangGraph demo comparing `graph_call` interrupt/resume checkpoints with `calls` mode LangChain model/tool checkpoints (no API keys) | [LangGraph Adapter](/guides/langgraph-adapter) |
...
<Callout type="info">
  ...
  API keys. The LangGraph adapter example is deterministic and local; check each
