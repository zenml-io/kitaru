...
| Compare LangGraph graph-call vs calls-mode checkpoints | `integrations/langgraph_agent/` | `uv run examples/integrations/langgraph_agent/langgraph_adapter.py --strategy graph_call` (local) or `--strategy calls` (requires `OPENAI_API_KEY`) |
