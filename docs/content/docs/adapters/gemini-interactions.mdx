...
A streamed run looks like this:
...
3. The adapter calls Gemini with stream=True.
...
| `GEMINI_STREAM_COMPLETED` | `gemini_interactions.stream.completed` | Kitaru finalized, persisted, and returned a stable result. This terminal event is flushed. |
...
accumulated step count, and the reconstruction policy. These are inspection
...
create a duplicate Gemini job. If the installed SDK does not support streamed
`get(...)`, Kitaru raises a feature-availability error when you call the stream
method; importing `kitaru.adapters.gemini` still works.
...
The adapter's Antigravity preset keeps `background=False` and `store=True`.
Google's public Antigravity surfaces have not been consistent about background
streaming support, so Kitaru does not promise Antigravity `background=True`
behavior here. For long Antigravity runs, use `timeout_s` to bound the provider
call and keep the task non-destructive unless you intentionally want the managed
agent to edit or execute things.
