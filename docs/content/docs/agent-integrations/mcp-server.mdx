...
`kitaru_executions_replay` is exposed, but replay support is still deferred in
`KitaruClient`. The tool returns a structured `available: false` response until
replay wrappers are implemented.
