...
<Callout type="info">
  This walkthrough uses `stage_5_typed_services.py` from the runnable Agent Factory example. If you have not cloned the repo yet, start with [Get the code](/agent-factory/#get-the-code) on the overview page.
</Callout>
...
- `agent_factory/services/schemas.py` holds the Pydantic models for each service's arguments and result (`LookupWikiArgs`, `LookupWikiResult`, `PublishSummaryArgs`, `PublishSummaryResult`, `WikiSnippet`).
- `agent_factory/services/registry.py` maps each service name to its `(args_model, handler, summary)` triple in `ALL_SERVICES`, and `build_service_description(allowed)` renders the tool description from the set of allowed services.
- `agent_factory/services/lookup_wiki.py` and `publish_summary.py` are the host-side handlers. Each resolves its credential with `kitaru.get_secret(...)` and makes one HTTP call to the mock.
- `agent_factory/tools.py` builds the `exec_service` tool. It validates `args` against the right Pydantic model on every call, so a malformed argument dict comes back to the agent as a `ValidationError` it can read instead of crashing inside the handler.
- `agent_factory/profile.py` adds the `allowed_services` field, which is what the tool description is built from.
...
- `mocks/runner.py` publishes the mock on a free host port and exports `AGENT_FACTORY_MOCK_BASE_URL` so the handlers can reach it.
