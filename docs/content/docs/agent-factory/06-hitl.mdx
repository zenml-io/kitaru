...
[Stage 5](/agent-factory/05-typed-services) narrowed what the agent can do. It reasons with `exec` inside the sandbox and takes specific actions with `exec_service`, and the profile decides which actions are even on the menu. That is a tight boundary. It still is not judgment.
...
<Callout type="info">
  This walkthrough uses `stage_6_hitl.py` from the runnable Agent Factory example. If you have not cloned the repo yet, start with [Get the code](/agent-factory/#get-the-code) on the overview page.
</Callout>
...
1. Look up `lookup_wiki(topic="durability")`, the host-side typed service from [Stage 5](/agent-factory/05-typed-services).
...
kitaru executions get <execution_id>   # status, turn output, wait history, artifacts
kitaru executions logs <execution_id>  # the runtime logs again
```
...
Kitaru did the part that makes that possible. The adapter routes `ask_question` through [`kitaru.wait()`](/guides/wait-and-resume), which writes a wait record to durable storage and lets the flow stop. Not block, stop. The process can exit, the compute can be handed back, and hours can pass with nothing running. When an answer finally arrives, from the terminal, the dashboard, the CLI, or a REST call, Kitaru loads the run back from that durable record and `ask_question(...)` returns the answer, as though it had been a slow function the whole time.

That is the difference from `input()`. `input()` keeps everything in one running process and bets that nothing interrupts it. `kitaru.wait()` writes down where the run is and then lets the process go. The four-hour wait costs you nothing while it waits, and a reboot in the middle does not lose the run.
...
```bash
kitaru executions replay <execution_id> --from default
```
...
- `agent_factory/tools.py` adds the `ask_question` tool. Its body calls `wait_for_input(question=..., name=...)` from the Kitaru PydanticAI adapter and coerces whatever comes back to a string for the agent's tool-result contract.
- `agent_factory/profile.py` already listed `ask_question` in its `ToolName` set; Stage 6 just turns it on in `allowed_tools`.
...
The one constraint to know about is the per-tool checkpoint wrapper. Stage 6 runs the same teaching setting as the rest of the tour, `granular_checkpoints=False`, so `ask_question` is not wrapped in its own tool checkpoint while the agent turn is running. That keeps the wait path simple for the tutorial.
...
If you drop the teaching-only `granular_checkpoints=False` and run granular (the upgrade [Stage 1](/agent-factory/01-durable-agent) describes), exempt `ask_question` from per-tool checkpoints by passing `tool_checkpoint_config_by_name={"ask_question": False}` to `KitaruAgent`. The other tools (`exec`, `skill`, `exec_service`) stay granular.

## Where this leaves us

That is the full Agent Factory. You started with a plain PydanticAI agent and a durable record of finished work, then added a sandbox, editable skills, a credential proxy, typed services, and now a durable pause for human judgment. Each layer narrowed what the agent can do on its own, and none of them made you rewrite the agent as a state machine.

Replay is not a stage of its own here. It is a general Kitaru primitive that works the same on any flow, not something specific to agent factories, so it lives in [Replay and Overrides](/guides/replay-and-overrides) rather than on its own Agent Factory page.

If you are weighing this pattern for real use, [Production notes and upgrade paths](/agent-factory/production-notes) gathers every teaching stand-in from the tour in one place and lists what to harden before you depend on it.
