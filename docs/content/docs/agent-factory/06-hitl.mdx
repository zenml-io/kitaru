...
Replay is not a stage of its own here. It is a general Kitaru primitive that works the same on any flow, not something specific to agent factories, so it lives in [Replay and Overrides](/guides/replay-and-overrides) rather than in its own chapter.
