...
<Callout type="info">
  This walkthrough uses `stage_4_credential_proxy.py` from the runnable Agent Factory example. If you have not cloned the repo yet, start with [Get the code](/agent-factory/#get-the-code) on the overview page.
</Callout>
...
First, three containers start on the `agent_factory` network:
...
`SandboxProxyRule` lives in `agent_factory/profile.py`. It says which hosts get which headers, and a header value can carry a `{{ secret-name.key }}` template. `build_credential_map` in `agent_factory/secrets.py` resolves those templates on the host and produces the `{host: {header: value}}` map the proxy needs. `DockerProxy` in `agent_factory/sandbox/proxy.py` takes that map, holds the per-run bearer that gates access, and runs the mitmproxy addon that matches each request's host and injects the header. If you ever swap mitmproxy for something else, `DockerProxy` and `SandboxProxyRule` are the two places to start. The CA generation and Docker network setup around them are plumbing you can leave alone.
