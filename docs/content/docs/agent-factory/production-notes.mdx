---
title: Production notes and upgrade paths
description: Which pieces of the Agent Factory tour are teaching stand-ins, where each one plugs into production, and what to harden before you rely on the pattern
---
...
The part worth keeping is the shape. Each capability sits behind a small, named seam, so you can replace the laptop-friendly version with something your platform team trusts without rewriting the agent. This page is the honest inventory: which pieces are teaching stand-ins, what they stand in for, and where you swap them.

The Kitaru primitives do not change: durability, wait/resume, and replay are the same concepts you would use in production. The production work is configuring the runtime and storage properly, then hardening the platform around those primitives. Everything else on this page lives in the example's forkable `agent_factory/` library, not in Kitaru itself.

<Callout type="warn">
  This example is a runnable reference architecture, not a hardened platform. As shipped it is not a hostile-code security boundary and not a turnkey production system. Treat the sections below as a starting checklist, not a guarantee.
</Callout>
...
|---|---|---|---|
| Durable execution (Stage 1) | `granular_checkpoints=False` for readable logs | The `KitaruAgent` per-call default | a flag, not a rewrite |
...
| Service credentials (Stage 4) | mitmproxy + self-signed CA on a shared Docker network | A real secret and network trust boundary | `SandboxProxyRule` plus the proxy container |
...
The durability story is identical either way. The only change is how finely work is cached and how finely it shows up on the dashboard. Nothing about Kitaru itself changes here. See [Stage 1](/agent-factory/01-durable-agent).

## Sandbox isolation

The `exec` tool depends on one method: `run(command) -> ExecResult`. `DockerSandbox` is the laptop implementation. Docker gives a real filesystem, process, and network boundary, which contains an agent that runs a bad `rm`. It is not a wall against code that is actively trying to break out, because the container shares the host's Linux kernel. When the commands matter, swap the backend at that one seam, roughly in order of how much isolation each one buys:

- **Docker** is the easy local default and stops accidental damage to the host.
- **gVisor** runs each container against a user-space kernel, so syscalls hit gVisor before the host kernel. Container-shaped UX, stronger isolation.
...
The agent's tool wiring does not change when you swap, because the swap happens behind `run(command)`. See [Stage 2](/agent-factory/02-sandbox).
...
Be clear about what Kitaru does and does not do here. Kitaru supplies the durable flow the read-and-act cycle runs inside, profile-gated access to the tool, and the swappable source seam. It does not ship skill versioning, review, diffs, change history, or any UI that surfaces edits. Those are real concerns, and a `SkillSource` subclass is where your fork adds them. See [Stage 3](/agent-factory/03-skills).
...
The seam is `SandboxProxyRule` plus the proxy container. See [Stage 4](/agent-factory/04-credential-proxy).
...
Adding a service is three files: a handler, an args and result Pydantic pair, and an `ALL_SERVICES` entry. The tool surface and its description update from the profile's `allowed_services`. See [Stage 5](/agent-factory/05-typed-services).

## Human approval and operator surface

On a laptop the operator answers `ask_question` at the same terminal. On a server the same `kitaru.wait()` record is answered through the dashboard, CLI, or REST API, and the non-interactive run in Stage 6 is already that shape. The durable pause already works the same way in production. The surface around it is where a real platform adds more:
...
See [Stage 6](/agent-factory/06-hitl).
...
4. **Input that crosses a trust boundary.** Validate and escape operator answers, model-authored commands, and service arguments before anything acts on them.
5. **Procedure governance.** Add the review, provenance, and version pinning that Kitaru does not ship for skills, on whichever `SkillSource` you choose.
6. **The platform around all of this.** Identity, policy, observability, deployment, and a production secret store are out of scope for the example. They are yours to bring.
