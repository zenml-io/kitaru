...
<Callout type="info">
  This walkthrough uses `stage_1_basic_agent.py` from the runnable Agent Factory example. If you have not cloned the repo yet, start with [Get the code](/agent-factory/#get-the-code) on the overview page.
</Callout>
...
- Before turn 2 starts, the process dies. Not because of a bug in your agent: a deploy restarts the worker, Kubernetes reschedules the pod, the box runs out of memory. Pick your poison.
- The job is retried from the top.

A plain agent loop kept turn 1's result in memory, and that memory died with the process. So the retry runs turn 1 again: the same model call, the same forty cents, the same twenty seconds, all to rebuild a result you already had a minute ago. On a ten-turn task that dies near the end, you pay for almost the entire run twice.
...
What PydanticAI does not do, and what no in-memory agent loop does on its own, is survive the process dying. Its progress lives in memory, so when the process goes, the progress goes with it.
...
```bash
DISABLE_CACHE=1 python stage_1_basic_agent.py
```
...
## Why `granular_checkpoints=False`

You will notice the wrapper passes `granular_checkpoints=False`. That is a teaching choice, not a default you would normally set.

With it off, each `agent.run_sync()` turn becomes one checkpoint (`default`, then `default_2`). That keeps the log output clean enough to point at while you are learning, and it lands the `FORCE_FAILURE` crash neatly on a turn boundary.

In a real fork you drop the kwarg. `KitaruAgent` defaults to `granular_checkpoints=True`, where every individual model request and every tool call gets its own checkpoint and its own cache key. A flow that crashed after the third model request then resumes by replaying only the calls after the third. The durability is the same, but the cache entries and dashboard rows are more precise.
