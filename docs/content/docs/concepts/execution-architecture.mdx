...
On remote stacks (Kubernetes, AWS, GCP, Azure), `runtime="isolated"` provisions a separate pod/job per invocation. Locally it falls back to inline so dev loops stay fast.
...
- The **runner** runs on whichever [stack](/stacks) you configure &mdash; Kubernetes, AWS, GCP, Azure, or local.
