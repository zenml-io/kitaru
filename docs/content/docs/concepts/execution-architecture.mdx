...
description: Server, runner, checkpoints, and execution targets — what actually happens when a Kitaru flow runs.
---
...
On remote stacks (Kubernetes, Vertex AI, SageMaker, AzureML), `runtime="isolated"` provisions a separate pod/job per invocation. Locally it falls back to inline so dev loops stay fast.
...
- The **runner** runs on whichever [stack](/stacks) you configure — Kubernetes, Vertex AI, SageMaker, AzureML, or local.
