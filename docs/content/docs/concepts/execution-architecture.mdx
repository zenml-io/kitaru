...
This split is the thing to remember. The control plane is long-lived and shared. The orchestration plane is per-run and durable. The execution plane is where your code (and your agent's code) actually executes.
...
**Inline** is the right default. It's fast, low-overhead, and makes sense for most orchestration logic. **Isolated** is the right call when a checkpoint:
...
- The **runner** runs on whichever [stack](/stacks) you configure — Kubernetes, AWS, GCP, Azure, or local.
- **Artifacts and state** live in your own S3 / GCS / Azure Blob bucket.

The Kitaru server brokers short-lived credentials when a client needs to read artifacts. There is no mandatory SaaS control plane in the path of your agent's data.
