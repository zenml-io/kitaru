...
A **flow** is the outer durable boundary in Kitaru &mdash; the unit of work
...
lives inside the checkpoints. Your platform sits in front of the flow&rsquo;s
invocation API. See
...
`.run()` starts an ad-hoc execution from the current process. It&rsquo;s the
