...
| `cache` | `None` | Whether checkpointed outputs can be reused from previous runs. `None` leaves the choice to each checkpoint's own `cache` setting; set `True` or `False` at the flow level to force it uniformly. |
