...
Everything inside the `@flow` boundary is durable. Checkpoints become persisted
work units; the rest is ordinary Python that orchestrates them.
...
`.run()` starts an ad-hoc execution from the current process &mdash; great for
iteration, local development, and calls from your own code.

For production use, flows are typically **deployed**: captured as an immutable,
versioned snapshot that consumers invoke through a stable URL, with tag-based
routing for rollout and rollback.
...
# Consumers invoke the deployed flow via the stable URL
handle = my_agent.invoke({"url": "https://example.com"})
```

See [Deployments](/concepts/deployments) for versioning, tag routing, rollback,
...
- Learn how to break work into durable units with
  [Checkpoints](/concepts/checkpoints)
- Ship a versioned, invokable flow with [Deployments](/concepts/deployments)
