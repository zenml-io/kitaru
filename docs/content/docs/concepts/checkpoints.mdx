...
Decorate work functions with `@kitaru.checkpoint`:
...
Call checkpoints from inside a `@kitaru.flow` to build your workflow:
...
| `type` | `None` | A label for dashboard visualization (e.g. `"llm_call"`, `"tool_call"`) |
...
```python
# This will raise RuntimeError — checkpoint called outside a flow
fetch_data("https://example.com")

# This will raise RuntimeError — nested checkpoint
@kitaru.checkpoint
def outer():
