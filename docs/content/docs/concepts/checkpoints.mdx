...
persisted. It&rsquo;s also the **contract between the runner and the
...
That separation is why a checkpoint failure is never just a crash &mdash;
it&rsquo;s persisted context the runner, agent loop, or a human can retry,
...
return their persisted outputs &mdash; execution only re-enters the first
incomplete one.

<CheckpointReplayDiagram />

You can also **override** a cached checkpoint&rsquo;s output during replay &mdash;
useful when you want to correct a single step&rsquo;s result and let the rest of
...
By default, checkpoints run **inline** &mdash; in the same process/pod as the
...
(Kubernetes, AWS, GCP, Azure). Locally it falls back to inline so dev loops
stay fast.
