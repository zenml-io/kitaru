...
`.submit()` returns a `KitaruStepFuture`. Call `.result()` on it to get the
checkpoint's output. This is the primary fan-out pattern in Kitaru.

<Callout type="info">
  `.submit()` returns a `KitaruStepFuture` — call `.result()` to collect the
  output, or `.running()` to check if the checkpoint is still in progress.
  Submit multiple checkpoints and collect their results later for fan-out /
  fan-in patterns.
</Callout>
...
Iterate a `KitaruMapFuture` to get individual `KitaruStepFuture` instances, or
call `.result()` to collect all outputs at once.
...
<Callout type="warn">
  ...
  **Dapr backend:** `runtime="isolated"` is ignored in the current MVP — all
  checkpoints run inline in the workflow worker process.
</Callout>
...
<Callout type="info">
  **Dapr backend:** Checkpoint retries are mapped to Dapr's native retry
  policy. When a checkpoint omits `retries`, it inherits the flow-level
  `retries` value as its default. Explicitly setting `retries=0` on a
  checkpoint disables retries even if the flow specifies a non-zero default.
</Callout>
