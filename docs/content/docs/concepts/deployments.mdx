...
- MCP: `deploy_flow(target="path/to/file.py:flow_name", ...)`
...
- MCP: `invoke_flow(flow="flow_name", ...)`
...
workspace/project connection that the CLI, SDK, and MCP server already use.

For a remote workspace, authenticate once and choose the project context:
...
`invoke_flow(...)` use that active workspace API key / auth context. The
...
Use `list_flow_deployments(flow="research_agent")` or
`get_flow_deployment(flow="research_agent", tag="stable")` when the assistant
