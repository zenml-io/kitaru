...
to use once you are there. If any piece is missing, commands that need the
server fail with a short error telling you what to set. `kitaru info` shows
which connection values Kitaru currently sees.
...
That bearer token is temporary; create or rotate long-lived automation
credentials with `kitaru auth api-keys`.
