...
A **deployment** is a reusable, versioned entrypoint for a Kitaru flow.
Think of it like pinning a recipe card to the wall: the flow source is the
recipe, a deployment version is one saved copy of that recipe, and an invocation
starts a fresh execution from that saved copy.
...
[Deploy and invoke flows guide](/guides/deployments).

When you deploy from source targets (`path.py:flow_name`) via CLI, run
`kitaru init` in the repository first so build/deploy-from-source metadata can
be resolved correctly.

## What gets saved

Deploying a flow creates a Kitaru-managed saved snapshot that Kitaru treats as
an immutable deployment version. Kitaru records the public flow name, an integer
version, deployment-time input defaults, the stack context, and any public
routing tags.

Deployment-time inputs are representative defaults. They let Kitaru prepare the
saved deployment snapshot, especially for flows whose shape depends on concrete
parameters. Later invocations can override those defaults by passing new inputs.
...
research_agent.deploy(topic="durable execution")
```
...
Tags are human-readable selectors that point at deployment versions. They are
how producers publish a route and consumers invoke it without memorizing version
numbers.
...
## Server-runnable stack requirement

Deployment creation is only supported for stacks that the Kitaru server can run
from a saved snapshot. If the selected stack is local or otherwise not
server-runnable, deployment creation is rejected (CLI, SDK, and MCP).
...
If you omit both `--version` and `--tag`, Kitaru invokes the `default` tag:
...
Those three values are the whole connection puzzle: where the Kitaru server is,
how to authenticate to it, and which project to use once you are there. If any
piece is missing, commands that need the server fail with a short error telling
you what to set. `kitaru info` shows which connection values Kitaru currently
sees.

After that, `kitaru invoke`, `KitaruClient().deployments.invoke(...)`, and MCP
`kitaru_deployments_invoke(...)` all use the active Kitaru server connection.
The invocation request does not carry a separate deployment-specific token.
...
requested tag or version first, then prints a command that starts a new
execution for that resolved deployment version. The generated snippet calls
`kitaru auth token` to get a short-lived server bearer token from your active
connection, but the curl generator itself does not inline real token values.
...
`kitaru flow deployments curl`) depends on server workload-manager support.
The official `zenmldocker/kitaru` image already enables this. If you run a
custom image or plain ZenML server setup, preserve or configure workload-manager
support explicitly (for example via
