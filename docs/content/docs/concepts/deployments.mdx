...
Deploy-time image config follows the same shape across CLI, SDK, and MCP: a
base image string or an object matching `kitaru.ImageSettings`.

Image config is part of the saved deployment snapshot. Later `kitaru invoke`
or `.invoke(...)` calls can override flow inputs for each execution, but they do
not rewrite the deployment image.

Each `kitaru deploy` call attaches exactly one routing tag at deploy time. If
you want to add another tag later, or move an existing route after testing, use
