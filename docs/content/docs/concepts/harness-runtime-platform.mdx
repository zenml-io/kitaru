...
Agent tooling spans four layers. Confusion between them is where most
...
- **Model layer** — the LLM itself. A compute unit over a context window, picked
  per-call or per-agent: OpenAI, Anthropic, Google, open-weights, fine-tuned in-house.
...
<Cards>
  <Card title="How It Works" href="/concepts/how-it-works" description="Server, runner, execution targets; three planes; local dev vs production." />
