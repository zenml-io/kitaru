...
## Summary
Issue #202 is primarily a Kitaru SDK monitoring gap for the tested `aws-k8s-stack`: `.run()` returned a `FlowHandle` promptly, `handle.wait()` then stayed silent, while `kitaru executions logs --follow --source step` streamed checkpoint logs live from the backend. The earlier synchronous-Kubernetes caveat remains relevant for other stack configurations, but the live reproduction proves the reported no-progress UX can be fixed inside Kitaru for this stack by reusing backend log following in the SDK wait path.
...
### Phase 4.5 - Live Kubernetes reproduction on `aws-k8s-stack`
**Hypothesis:** If backend step logs stream live through the existing CLI while SDK `handle.wait()` stays silent, then the main fix for the tested stack is Kitaru-side SDK monitoring rather than ZenML log-store freshness.
**Findings:** Confirmed. A disposable `remote_log_probe` flow was run on `aws-k8s-stack` with two 90-second noisy checkpoints. `.run()` returned a `FlowHandle` quickly (`exec_id=0b4e023e-d66f-4b20-ae5b-9bc6eb39b2a7`) after 13.1 seconds, then Terminal A printed `LOCAL before handle.wait()` and stayed silent during remote checkpoint execution. In parallel, `kitaru executions logs <exec_id> --follow --source step --interval 2 -v` streamed `checkpoint=alpha` and `checkpoint=bravo` tick logs live roughly every 10 seconds and exited with `[Execution completed successfully]`. `--source runner --follow` emitted no runtime log lines, only the completion control message.
**Evidence:** `/tmp/kitaru-issue-202-terminal-a.log`, `/tmp/kitaru-issue-202-step-follow.log`, `/tmp/kitaru-issue-202-runner-follow.log` from the 2026-04-24 05:54 UTC run. Terminal A showed `remote_log_probe.run() returned FlowHandle ... after 13.1s` and then no checkpoint logs during `handle.wait()`. The step-follow log showed live entries from `checkpoint=alpha START` through `checkpoint=bravo END` before terminal completion.
**Conclusion:** For the tested Kubernetes stack, the backend exposes step logs incrementally and the existing Kitaru CLI follow loop can stream them. The proper fix for this reproduction is inside Kitaru: make SDK waiting reuse a shared backend log-follow/progress monitor. The pre-handle synchronous-Kubernetes concern should remain in the design notes, but it is not the blocking cause on `aws-k8s-stack`.
**Incidental finding:** After completion, the probe's `handle.wait()` raised `KitaruRuntimeError` because output metadata was missing and fallback extraction was ambiguous with multiple terminal steps. This is separate from log streaming and likely caused by the disposable probe shape, but it may deserve a separate small investigation if seen outside the probe.

## Root Cause
Kitaru's Python SDK currently does not own live log following for remote `.run()` executions. The live `aws-k8s-stack` reproduction narrows the main cause: backend step logs are available live through the CLI, but SDK `handle.wait()` does not consume them. There are two architectural layers to keep in mind:
...
- Backend log freshness was verified for `aws-k8s-stack`: `/steps/{step_id}/logs` data is available incrementally through `kitaru executions logs --follow --source step`. So for this stack there is no ZenML/log-store freshness blocker; Kitaru's missing SDK follow path is the main issue.
...
3. **Keep the pre-handle synchronous Kubernetes window as a design guardrail, but prioritize SDK wait-follow for this stack.** On `aws-k8s-stack`, `.run()` returned a `FlowHandle` in 13.1 seconds, so `FlowHandle.wait()` is the immediate fix location. Still, if another remote stack blocks inside `configured_pipeline(...)`, a `wait()`-only change would be too late; the design should avoid relying on ZenML's synchronous pod-log monitor where possible.
