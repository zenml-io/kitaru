...
// ============================================================
// Diagram 8: Versioned deployments + tag router
// ============================================================

export function DeploymentsDiagram() {
  const versionBox = (v: string, tag?: string, tone: Tone = 'default') => (
    <Node
      key={v}
      title={v}
      subtitle={tag ? `tag: ${tag}` : undefined}
      tone={tone}
      isMono
      minWidth={130}
    />
    ...
    <DiagramFrame>
      <div style={{ ...col, gap: 14 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <Node
            title="flow.deploy()"
            subtitle="build image · capture snapshot · register next version"
            tone="default"
            isMono
            minWidth={340}
          />
        ...
        <Subgraph label="Immutable versions" tone="accent" annotation="kept forever · all invokable">
          <div style={{ ...row, justifyContent: 'center' }}>
            ...
            {versionBox('coding-agent v3', 'canary', 'warn')}
            {versionBox('coding-agent v4')}
          </div>
        </Subgraph>

        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <VArrow label="tag / version routing · moves are instant" />
        </div>

        {/* Stable URL */}
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <Node
            title="POST /invoke/coding-agent"
            subtitle="one stable URL per flow · tag or version resolved server-side"
            tone="info"
            isMono
            minWidth={400}
          />
        ...
        <div style={{ ...row, justifyContent: 'center' }}>
          <Node
            title="default consumers"
            subtitle="no tag → v2"
            tone="muted"
            minWidth={180}
          />
          <Node
            title="canary consumers"
            subtitle="tag=canary → v3"
            tone="muted"
            minWidth={200}
          />
          <Node
            title="pinned consumer"
            subtitle="version=1 → v1"
            tone="muted"
            minWidth={190}
          />
        </div>
      ...
      <Caption>
        Rollback is a tag move, not a redeploy. Moving <code>default</code> from
        v2 to v1 reroutes traffic instantly — no rebuild, no downtime.
      </Caption>
    </DiagramFrame>
  );
}
...
// ============================================================
// Diagram 9: Invocation modes — serverless vs served
// ============================================================

export function InvocationModesDiagram() {
  return (
    <DiagramFrame>
      <div style={{ ...col, gap: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'center' }}>
        ...
        >
          <Node
            title="Serverless"
            subtitle="schedule or cold-start a runner"
            bullets={[
              'long-horizon agents',
              'batch tasks',
              'cold-start tolerant',
          ...
          <Node
            title="Served"
            subtitle="hot long-lived runtime"
            bullets={[
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <VArrow label="same durable runtime in both modes" />
        </div>
        <Subgraph label="Kitaru durable runtime" tone="warn">
          <div style={{ ...row, justifyContent: 'center' }}>
            <Node title="checkpoints" tone="warn" isMono minWidth={140} />
            <Node title="replay" tone="warn" isMono minWidth={100} />
            <Node title="resume" tone="warn" isMono minWidth={110} />
            <Node title="wait" tone="warn" isMono minWidth={100} />
            <Node title="artifacts + state" tone="warn" isMono minWidth={160} />
          </div>
        </Subgraph>
      </div>
      <Caption>
        Invocation mode controls latency and footprint. Durability is the same
        either way.
      </Caption>
    </DiagramFrame>
  );
...
// ============================================================
// Diagram 10: Gateway stack — fits behind your platform
// ============================================================
