...
- **[Evaluators](evaluators.md)** score sessions and write evaluations — typed, versioned verdicts. Human labels land in the same table.
- **[Cohorts](cohorts.md)** freeze a population of sessions into immutable versions, so results stay comparable.
- **[Experiments](experiments.md)** replay a cohort against a change and score both sides — what improved, what regressed, before you ship.
- **[Investigations](investigations.md)** structure human review: questions asked over a set of sessions, answers stored as **annotations** pinned to exact trace locations — distinct from evaluations, and the raw material for calibrating them.
