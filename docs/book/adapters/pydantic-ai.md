...
- `KITARU_TASK_INPUTS` (or the task spec) carries the baseline's recorded inputs — the adapter **substitutes them for your script's own prompt**, which is why a hardcoded prompt in `__main__` is fine.
...
- **Import-first alternative:** the [ticket-resolver example](https://github.com/zenml-io/kitaru/tree/develop/examples/pydantic_ai_ticket_resolver) imports a checked-in Langfuse export of PydanticAI runs into Kitaru as replayable sessions.
