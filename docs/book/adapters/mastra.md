...
| `file_url_undeclared` | A file or image part in the input holds a network URL that `files` did not declare, thread history holds one and no `resolveFile` was supplied, a processor passed the factory's `resolveFile` a URL that is neither declared nor in a file or image part of thread history, or observational memory read a history file that no processor resolved, which Mastra downloads itself when the observer or reflector model cannot read its URL. |
