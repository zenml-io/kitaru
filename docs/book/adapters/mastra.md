...
| `file_url_undeclared` | A file or image part in the input or thread history holds a network URL that `files` did not declare, or a processor passed such a URL to the factory's `resolveFile`. |
