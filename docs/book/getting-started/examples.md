...
| `integrations/agentu_agent/agentu_record_replay.py` | Make a plain agentu workflow durable and forkable: researcher fan-out → analyst → writer as checkpoints, an agentu middleware that saves every inner LLM prompt/response as Kitaru artifacts, and two replay forks of the writer step — counterfactual analysis and model swap (OpenAI-compatible endpoint required; Ollama by default) | [Replay and Overrides](../guides/replay-and-overrides.md) |
