...
`harbor` is the Harbor CLI. `dataset` is the generated task dataset. `agent.kitaru_agent:KitaruAgent` is a fixed adapter generated by Kitaru, not the user's agent name or class. Harbor supplies the task sandbox, and the adapter runs the registered user agent command and working directory unchanged. It then copies the declared trace to Harbor's agent logs. The verifier converts that trace into an evaluator-facing Kitaru session, runs all pinned evaluators, writes the selected reward to `/logs/verifier/reward.txt`, and writes numeric results to `/logs/verifier/metrics.json`.
...
uv run eval \
  ...
  --env.agent.harness.forward-env TOOLS_API_KEY
```

`uv sync` installs the generated project and its pinned Verifiers dependency. `eval` is the Verifiers CLI, not a Kitaru command. `kitaru-verifiers-v1` is the fixed local taskset package generated by Kitaru, not the user's agent name. Replace `MODEL` with the model identifier to evaluate. The user's exported code lives under `agent_source/`, and the generated Kitaru harness runs its registered command inside the selected Verifiers runtime.
