...
A replayed agent needs the same credentials the original had — a model provider key, a database URL. You could bake them into every worker's environment; **secrets** are the managed alternative: named bundles of key-value pairs, stored encrypted on the server (`KITARU_SERVER_SECRET_ENCRYPTION_KEY`), and injected into agent subprocesses at run time.
