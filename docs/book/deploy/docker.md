...
The server is one container plus Postgres. Compose runs both on a single
...
The server is configured entirely through `KITARU_SERVER_*` environment
variables. The ones every deployment should set:
...
Then create accounts and API keys for the team:
