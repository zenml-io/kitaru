...
Supported tool policies are passthrough, static, and history, including `fail`, `passthrough`, and `error_result` miss behavior. History lookup computes the same SHA-256 cache key as the Python server from the tool name and JSON inputs. The current lookup response cannot distinguish a successful `null` result from a failed recorded call, so a found `null` fails closed regardless of `on_miss`; the adapter neither mocks success nor executes the tool. The `llm` policy fails before tool execution because it is not supported in this release.
