...
def test_ci_canonical_example_job_enforces_the_walkthrough() -> None:
    workflow = (REPO_ROOT / ".github" / "workflows" / "ci.yml").read_text()
    assert "\n  canonical-example:\n" in workflow
    ...
    assert "name: Canonical example end to end" in example_job
    ...
    assert "Install current Kitaru artifacts into canonical example" in example_job
...
def test_each_unit_exposes_its_exact_release_critical_checks() -> None:
    ...
    assert "Canonical example end to end" in inventory.common_checks
