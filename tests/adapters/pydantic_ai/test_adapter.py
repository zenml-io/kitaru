...
class _FakeSessions:
    def __init__(self, client: "_FakeClient") -> None:
        ...
        self.updated: list[tuple[uuid.UUID, SessionUpdateRequest]] = []
        self.ingested: list[tuple[uuid.UUID, SessionNodeBatchRequest]] = []
    ...
    async def ingest_nodes(
        ...
        self.ingested.append((session_id, request))
...
class _FakeReplays:
    def __init__(self, client: "_FakeClient") -> None:
        self._client = client
        self.lookups: list[tuple[uuid.UUID, ToolLookupRequest]] = []

    async def get(self, replay_id: uuid.UUID) -> ReplayResponse:
        assert self._client.spec is not None
        assert replay_id == self._client.spec.replay.id
        return self._client.spec.replay
...
class _FakeClient:
    ...
    next_spec: ClassVar[_ReplayFixture | None] = None
...
def _nodes(client: _FakeClient) -> list[Any]:
    return [node for _, batch in client.sessions.ingested for node in batch.nodes]


def _history_policy() -> HistoryConfig:
...
async def test_setup_error_fails_created_session(
    ...
    _ = monkeypatch
    _FakeClient.next_ingest_error = RuntimeError("node ingest failed")
    agent = KitaruAgent(
    ...
    with pytest.raises(RuntimeError, match="node ingest failed"):
...
async def test_replay_uses_task_accessor_when_environment_input_is_absent(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    seen_prompts: list[Any] = []
    ...
    monkeypatch.setattr(capability_module, "get_task_inputs", lambda: spec.inputs)
    agent = KitaruAgent(
        Agent(FunctionModel(model)),
        agent_id=uuid.uuid4(),
        api_url="http://kitaru.test",
...
async def test_non_replay_run_preserves_model_settings() -> None:
    ...
    assert observed_settings == [{"temperature": 0.9, "max_tokens": 100}]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (_history_policy(), {"source": "history"}, 0, True),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (_static_policy(cases=[StaticCase(result={"mocked": True})]), False),
        (_history_policy(), False),
...
async def test_history_policy_normalizes_validated_tool_arguments(
    ...
    spec = _replay_spec(_history_policy())
    _set_replay(monkeypatch, spec)
    _FakeClient.next_lookup_response = ToolLookupResponse(
