...
"""Round-trip tests for the users SDK resource."""
...
async def test_create(api_client: KitaruAPIClient) -> None:
    """Create a user through the SDK."""
    account = await api_client.users.create(
...
async def test_create_duplicate_name(api_client: KitaruAPIClient) -> None:
    ...
    await api_client.users.create(UserCreateRequest(name="alice"))
...
async def test_deactivate(api_client: KitaruAPIClient) -> None:
    """Deactivate a user through the SDK."""
    created = await api_client.users.create(UserCreateRequest(name="alice"))
    deactivated = await api_client.users.deactivate(created.id)
