...
async def test_create(
    ...
    """Create a session run job through the SDK."""
    ...
    job = await api_client.session_runs.create(
        SessionRunCreateRequest(agent_version_id=version.id, inputs={"q": "hi"})
    )
    assert isinstance(job, JobResponse)
    assert job.status.value == "pending"


async def test_create_not_found(api_client: KitaruAPIClient) -> None:
    """Surface HTTP 404 as a typed error for an unknown agent version."""
    with pytest.raises(NotFoundError):
        await api_client.session_runs.create(
            SessionRunCreateRequest(agent_version_id=uuid.uuid4(), inputs=None)
        )


async def test_create_rejects_a_version_without_a_run_spec(
    api_client: KitaruAPIClient, services: JobAndTaskServices
) -> None:
    """Surface HTTP 422 as a typed error when the version carries no run spec."""
    agent = await create_agent(services.agents, ACCOUNT.id)
    version = await create_agent_version(
    ...
    with pytest.raises(ValidationError):
        await api_client.session_runs.create(
            SessionRunCreateRequest(agent_version_id=version.id, inputs=None)
        )
