...
async def run_request(services: ReplayServices) -> ExperimentRunCreateRequest:
    """Provide a run request naming a non-empty cohort and a runnable version."""
    ...
    return ExperimentRunCreateRequest(cohort_id=cohort.id, agent_version_id=version.id)
...
async def test_start_run_rejects_empty_cohort(
    ...
    """Surface HTTP 422 as a typed error for an empty cohort."""
    ...
    with pytest.raises(APIError) as excinfo:
        await api_client.experiments.start_run(
            experiment_id,
            ExperimentRunCreateRequest(
                cohort_id=empty_cohort.id, agent_version_id=uuid.uuid4()
            ),
        )
