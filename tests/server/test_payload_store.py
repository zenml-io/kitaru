...
async def test_offload_dedupes_identical_values() -> None:
    ...
    fakes = build_payload_store(threshold_bytes=10)
    shared_value = {"a": "i" * 50}
    first = Payload.json(shared_value)
    second = Payload.json(shared_value)
...
async def test_offload_keeps_byte_identical_values_apart_by_media_type() -> None:
    ...
    text_payload = Payload.text(f'"{padded}"')
    json_payload = Payload.json(padded)
    ...
    resolved_text = Payload.ref(text_payload.blob_id)
    resolved_json = Payload.ref(json_payload.blob_id)
