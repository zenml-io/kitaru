...
async def test_create_replay_rejects_cohort_scoped_history(
    services: ReplayServices,
) -> None:
    """A standalone replay cannot use cohort-scoped history."""
    agent_version = await _agent_version(services)
    await _evaluator(services)
    baseline = await _session(services, agent_version)
    tool_policy = ToolPolicy(
        default=HistoryConfig(scope=HistoryScope.COHORT, on_miss=ToolPolicyOnMiss.FAIL)
    )
    with pytest.raises(ValidationError, match="cannot use cohort-scoped history"):
        await services.replay_service.create_replay(
            ReplayCreate(
                baseline_session_id=baseline.id,
                evaluators=[EvaluatorConfigInput(evaluator="accuracy")],
...
async def test_tool_lookup_cohort_scope_within_the_runs_cohort(
    services: ReplayServices,
) -> None:
    """Cohort scope matches nodes only within the run's cohort."""
    agent_version = await _agent_version(services)
    baseline = await _session(services, agent_version)
    cohort_member = await _session(services, agent_version)
    outside_cohort = await _session(services, agent_version)
    ...
    await services.session_nodes.upsert_batch(
        cohort_member.id,
        [_cache_node(cohort_member.id, 0, cache_key, {"result": "in-cohort"})],
    )
    await services.session_nodes.upsert_batch(
        outside_cohort.id,
        [_cache_node(outside_cohort.id, 0, cache_key, {"result": "outside"})],
    )
    ...
    replay_id = await _replay_with_history_scope(
        services, HistoryScope.COHORT, baseline, experiment_run_id=run_id
    )
    ...
    assert result.found is True
    assert result.result == {"result": "in-cohort"}
