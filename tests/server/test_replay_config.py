...
def test_check_standalone_rejects_cohort_history_on_default() -> None:
    """Reject a standalone replay whose default config scopes to cohort."""
    policy = ToolPolicy(
        default=HistoryConfig(scope=HistoryScope.COHORT, on_miss=ToolPolicyOnMiss.FAIL)
    )
    config = _config(policy)
    with pytest.raises(ValidationError, match="cohort-scoped history"):
        config.check_standalone()


def test_check_standalone_rejects_cohort_history_on_named_tool() -> None:
    """Reject a standalone replay whose per-tool config scopes to cohort."""
    policy = ToolPolicy(
        default=PassthroughConfig(),
        tools={
            "search": HistoryConfig(
                scope=HistoryScope.COHORT, on_miss=ToolPolicyOnMiss.FAIL
            )
        },
    )
    config = _config(policy)
