...
async def test_list_tags_walks_pages_with_cursor(client: httpx.AsyncClient) -> None:
    ...
    while True:
        response = await client.get("/v1/tags", params=params)
        assert response.status_code == 200
        body = response.json()
...
async def test_update_tag_not_found(client: httpx.AsyncClient) -> None:
    ...
    response = await client.patch(
        f"/v1/tags/{uuid.uuid4()}", json={"name": "production"}
    )
...
async def test_update_tag_conflict(client: httpx.AsyncClient) -> None:
    ...
    await client.post("/v1/tags", json={"name": "prod"})
    staging = (await client.post("/v1/tags", json={"name": "staging"})).json()
    response = await client.patch(f"/v1/tags/{staging['id']}", json={"name": "prod"})
    assert response.status_code == 409
...
async def test_delete_tag(client: httpx.AsyncClient) -> None:
    ...
    response = await client.patch(f"/v1/tags/{created['id']}", json={"name": "x"})
    assert response.status_code == 404


async def test_delete_tag_not_found(client: httpx.AsyncClient) -> None:
    """Observe HTTP 404 for an unknown tag id."""
    response = await client.delete(f"/v1/tags/{uuid.uuid4()}")
    assert response.status_code == 404
...
async def test_delete_tag_link_not_found(client: httpx.AsyncClient) -> None:
    ...
    created = (await client.post("/v1/tags", json={"name": "prod"})).json()
    response = await client.delete(
        f"/v1/tags/{created['id']}/links/session/{uuid.uuid4()}"
    )
    assert response.status_code == 404
