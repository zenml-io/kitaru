...
def test_modal_backend_with_modal_settings() -> None:
    ...
    assert settings.modal.image == "zenmldocker/kitaru-worker:1.0.0"
...
def test_modal_settings_parsed_from_env_vars(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv(
        "KITARU_SERVER_WORKER_LAUNCHER__MODAL__IMAGE",
        "zenmldocker/kitaru-worker:1.0.0",
    )
    monkeypatch.setenv("KITARU_SERVER_WORKER_LAUNCHER__MODAL__TIMEOUT_SECONDS", "120")
    ...
    assert settings.WORKER_LAUNCHER.modal.image == "zenmldocker/kitaru-worker:1.0.0"
    assert settings.WORKER_LAUNCHER.modal.timeout_seconds == 120
