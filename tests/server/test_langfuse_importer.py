...
import kitaru_importer_langfuse.importer as importer_module
import pytest
from kitaru_importer_langfuse import LangfuseJSONLImporter, parse
from kitaru_importer_langfuse.models import (
    ImportContext,
    InvalidImport,
)
...
def test_unified_parse_takes_params_as_a_positional_dict() -> None:
    """Honor the job spec calling convention of ``call_parser``."""
    parsed = parse(
        jsonl(observation("root", "trace-1")),
        {"source_instance": "langfuse-eu", "filename": "export.jsonl"},
    )

    assert len(parsed) == 1
    assert isinstance(parsed[0], ParsedSession)
    assert parsed[0].external_id.startswith("langfuse-eu:")


def test_unified_parse_ignores_unknown_params() -> None:
    """Tolerate job params that a released plugin does not understand."""
    parsed = parse(jsonl(observation("root", "trace-1")), {"unknown": "value"})

    assert len(parsed) == 1


def test_rejects_payload_over_upload_limit(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(importer_module, "MAX_UPLOAD_BYTES", 4)

    with pytest.raises(InvalidImport, match="upload limit"):
        parse(b"12345")


def test_package_entrypoint_satisfies_current_importer_contract() -> None:
    """Load and advance the installed package through the task plugin contract."""
    entrypoint = load_source_ref("kitaru_importer_langfuse:parse", "Importer")

    parsed = list(
        call_parser(
            entrypoint,
            jsonl(observation("root", "trace-1")),
            {"source_instance": "langfuse-eu"},
        )
    )

    assert len(parsed) == 1
    assert isinstance(parsed[0], ParsedSession)
    assert parsed[0].external_id == "langfuse-eu:conversation-1"
...
def test_single_trace_row_without_observation_id_uses_trace_id() -> None:
    """Accept trace-only exports that omit a separate root observation id."""
    batch = LangfuseJSONLImporter().parse(
        jsonl(
            {
    ...
    assert batch.errors == []
    session = batch.sessions[0]
    assert session.source_id == "trace-only-1"
    assert len(session.nodes) == 1
    assert session.nodes[0].source_id == "trace-only-1:trace-only-1"
