...
async def test_start_run_creates_one_replay_per_cohort_session_in_order(
    services: ReplayServices,
) -> None:
    ...
    run, counts = await services.experiment_service.start_run(
        experiment_id,
        ExperimentRunCreate(cohort_id=cohort.id, agent_version_id=agent_version.id),
        actor=ACTOR,
    )
    assert run.number == 1
    ...
    ordered = sorted(bundles, key=lambda bundle: bundle.replay.id)
    assert [bundle.replay.baseline_session_id for bundle in ordered] == [
        session.id for session in sessions
    ]
...
async def test_start_run_rejects_an_empty_cohort(services: ReplayServices) -> None:
    """An empty cohort rejects the run before any job is created."""
    agent_version = await _agent_version_with_run_spec(services)
...
async def test_run_numbers_increment_per_experiment(services: ReplayServices) -> None:
    ...
    cohort_a = await create_cohort(
        services.cohorts, ACTOR.account.id, agent_version.agent_id, [session_a.id]
    )
    ...
    run_1, _ = await services.experiment_service.start_run(
        experiment_id,
        ExperimentRunCreate(cohort_id=cohort_a.id, agent_version_id=agent_version.id),
        actor=ACTOR,
    )
    run_2, _ = await services.experiment_service.start_run(
        experiment_id,
        ExperimentRunCreate(cohort_id=cohort_b.id, agent_version_id=agent_version.id),
        actor=ACTOR,
    )
...
async def test_duplicate_replay_for_the_same_baseline_in_a_run_conflicts(
    ...
    cohort = await create_cohort(
        services.cohorts, ACTOR.account.id, agent_version.agent_id, [session.id]
    )
