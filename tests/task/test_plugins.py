...
def test_load_plugin_entrypoint_import_error(tmp_path: Path) -> None:
    ...
    assert "kitaru._task_plugin" not in sys.modules
...
def test_load_plugin_module_registers_before_execution(tmp_path: Path) -> None:
    """Register the module in sys.modules before executing its top-level code."""
    path = tmp_path / "plugin.py"
...
@pytest.mark.parametrize(
    "ref", ["module", "module:", ":attribute", "module:attribute:extra"]
)
def test_parse_source_ref_rejects_invalid_format(ref: str) -> None:
    """Raise ValueError for anything but a single module:attribute pair."""
