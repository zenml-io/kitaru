...
"""Evaluator task process handler."""
...
class EvaluationHandler:
    """Build evaluator task subprocesses."""
    ...
    async def prepare(
        ...
        """Build an evaluator subprocess.

        Args:
            ctx: Shared execution dependencies.
            task_id: Claimed task id.
            spec: Evaluator task specification.
        ...
        if not isinstance(details, EvaluationTaskDetails):
            raise ValueError("Evaluator task has mismatched details.")
        ...
        if isinstance(plugin, ScriptPluginSpec):
            ...
            raise ValueError(f"Unsupported evaluator plugin: {plugin!r}")
        return TaskProcess(
