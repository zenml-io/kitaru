...
**Safety model.** The model-facing tool accepts only `command` and optional `cwd`; `env` is deliberately not exposed because captured tool arguments may be stored as artifacts. The model still controls the shell command and working directory. Anything the sandbox process can read can be printed to `stdout` or `stderr`; that output is returned to the model and, with calls-strategy checkpoints, persisted in Kitaru execution/checkpoint artifacts. If the sandbox can see files, environment variables, network access, or credentials, the model may cause a command to reveal them. The `local` sandbox is a development convenience, not a security boundary: it runs local subprocesses with access available to your user. For untrusted models or prompts, use an isolated sandbox provider and pass only the credentials that command truly needs.
