...
class _KitaruCapability(AbstractCapability[Any]):
    ...
    async def for_run(self, ctx: RunContext[Any]) -> "_KitaruCapability":
        ...
        try:
            ...
            effective_input = get_task_inputs() if task_id else ctx.prompt
            effective_input_json = _jsonable(effective_input)
        ...
        try:
            session = await state.client.sessions.create(
                SessionCreateRequest(
                    ...
                    inputs=state.effective_input_json,
    ...
    async def wrap_model_request(
        ...
        *,
        ...
        override = state.replay.override if state.replay is not None else None
            ...
            if policy is None or isinstance(policy, PassthroughConfig):
                ...
                if state.replay is None:
                    raise ToolPolicyError(
                        "History tool policy requires KITARU_REPLAY_ID"
                    )
    ...
    def _tool_policy(self, tool_name: str) -> ToolConfig | None:
        ...
        if state.replay is None:
            return None
        config: ToolPolicy = state.replay.tool_policy
    ...
    async def _allocate_node(self) -> int:
        """Allocate a monotonic session-local node index."""
        state = self._require_state()
    ...
    async def _buffer_node(self, node: SessionNodeCreateRequest) -> None:
        ...
        async with state.lock:
            ...
            should_flush = len(state.buffer) >= self.batch_size
        if should_flush:
            await self._flush(state)

    async def _flush(self, state: _RunState) -> None:
        """Flush one ordered snapshot without holding the state lock during I/O."""
        async with state.flush_lock:
            async with state.lock:
                ...
                batch = SessionNodeBatchRequest(
                    nodes=sorted(state.buffer[:count], key=lambda node: node.index)
                )
    ...
    async def _finish(
        ...
        *,
        ...
        state.finished = True
        ended_at = datetime.now(UTC)
        outputs_json = _jsonable(outputs)
        async with state.lock:
            state.buffer.append(
                SessionNodeCreateRequest(
                    index=0,
