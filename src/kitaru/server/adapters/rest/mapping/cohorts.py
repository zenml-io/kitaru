...
def cohort_sessions_list_params_to_filter(
    cohort_id: uuid.UUID, params: CohortSessionsListParams
) -> CohortSessionsFilter:
    """Convert list params to the application filter scoped to one cohort.

    Args:
        cohort_id: Id of the cohort whose sessions to list.
        params: Cohort sessions list params.

    Returns:
        Cohort sessions filter.
    """
