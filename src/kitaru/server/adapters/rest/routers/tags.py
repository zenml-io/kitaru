...
async def list_tags(
    ...
    Returns:
        Page of tags.
    """
    tag_filter = TagFilter(name=name, page=page, page_size=page_size)
    tags, total = await service.list_tags(tag_filter, actor=actor)
...
async def delete_tag(
    ...
    Clients observe HTTP 204 on success and 404 when no tag has this id.
...
async def create_tag_link(
    ...
    when the tag is already attached to the resource, and 422 on invalid
    input. The referenced resource is not validated to exist.
    ...
    link = await service.create_tag_link(
        tag_id,
        resource_type=resource_type_to_domain(body.resource_type),
...
async def delete_tag_link(
    ...
    Clients observe HTTP 204 on success, 404 when the tag is not attached
    to the resource, and 422 on an invalid resource type.
    ...
    await service.delete_tag_link(
        tag_id,
        resource_type=resource_type_to_domain(resource_type),
