...
orchestration functions in ``mapping/plugins.py``, since evaluators and
...
from kitaru.server.adapters.rest.mapping import plugins as plugin_mapping
...
async def create_importer(
    ...
    Returns:
        Created importer.
    """
    return await plugin_mapping.create_plugin(
        service, body, ImporterResponse, actor=actor
    )
...
async def list_importers(
    ...
    Returns:
        Page of importers.
    """
    return await plugin_mapping.list_plugins(
        service,
        params,
        ImporterResponse,
        actor=actor,
...
async def get_importer(
    ...
    Returns:
        Stored importer.
    """
    return await plugin_mapping.get_plugin(
        service, importer_id, ImporterResponse, actor=actor
    )
...
async def update_importer(
    ...
    Returns:
        Updated importer.
    """
    return await plugin_mapping.update_plugin(
...
async def delete_importer(
    ...
    Args:
        ...
        actor: Caller context.
    """
    await plugin_mapping.delete_plugin(service, importer_id, actor=actor)
...
async def create_importer_version(
    ...
    """
    return await plugin_mapping.create_version(
        service,
        importer_id,
        body.source,
...
async def list_importer_versions(
    ...
    """
    return await plugin_mapping.list_versions(
...
async def get_importer_version(
    ...
    """
    return await plugin_mapping.get_version(
...
async def update_importer_version(
    ...
    """
    return await plugin_mapping.update_version(
        service,
        importer_id,
        version,
        body.display_version,
