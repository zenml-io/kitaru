...
def downgrade() -> None:
    ...
    with op.batch_alter_table("task", schema=None) as batch_op:
        batch_op.drop_constraint(TASK_PAYLOAD_BLOB_ID_FOREIGN_KEY, type_="foreignkey")
        batch_op.create_foreign_key(
            TASK_PAYLOAD_BLOB_ID_FOREIGN_KEY, "blob", ["payload_blob_id"], ["id"]
        )
        batch_op.drop_constraint(TASK_INPUT_SESSION_ID_FOREIGN_KEY, type_="foreignkey")
        batch_op.create_foreign_key(
            TASK_INPUT_SESSION_ID_FOREIGN_KEY, "session", ["input_session_id"], ["id"]
        )
        batch_op.drop_constraint(TASK_PLUGIN_VERSION_ID_FOREIGN_KEY, type_="foreignkey")
        batch_op.create_foreign_key(
            TASK_PLUGIN_VERSION_ID_FOREIGN_KEY,
        ...
        batch_op.drop_constraint(TASK_AGENT_ID_FOREIGN_KEY, type_="foreignkey")
        batch_op.create_foreign_key(
            TASK_AGENT_ID_FOREIGN_KEY, "agent", ["agent_id"], ["id"]
        )
    ...
    with op.batch_alter_table("session", schema=None) as batch_op:
        ...
        batch_op.drop_constraint(SESSION_AGENT_ID_FOREIGN_KEY, type_="foreignkey")
        batch_op.create_foreign_key(
            SESSION_AGENT_ID_FOREIGN_KEY, "agent", ["agent_id"], ["id"]
        )

    with op.batch_alter_table("investigation", schema=None) as batch_op:
        batch_op.drop_constraint(INVESTIGATION_AGENT_ID_FOREIGN_KEY, type_="foreignkey")
        batch_op.create_foreign_key(
            INVESTIGATION_AGENT_ID_FOREIGN_KEY, "agent", ["agent_id"], ["id"]
        )
    ...
    with op.batch_alter_table("cohort_version", schema=None) as batch_op:
        batch_op.drop_constraint(
            COHORT_VERSION_OWNER_ID_FOREIGN_KEY, type_="foreignkey"
        )
    ...
    with op.batch_alter_table("cohort", schema=None) as batch_op:
        ...
        batch_op.drop_constraint(COHORT_OWNER_ID_FOREIGN_KEY, type_="foreignkey")

    with op.batch_alter_table("agent_version", schema=None) as batch_op:
        batch_op.drop_constraint(AGENT_VERSION_AGENT_ID_FOREIGN_KEY, type_="foreignkey")
        batch_op.create_foreign_key(
            AGENT_VERSION_AGENT_ID_FOREIGN_KEY, "agent", ["agent_id"], ["id"]
        )

    # tag_link's typed columns cannot round-trip back into a single
    # resource_type/resource_id pair, so existing rows are dropped rather
    # than converted.
    with op.batch_alter_table("tag_link", schema=None) as batch_op:
        batch_op.drop_index(TAG_LINK_TAG_ID_INDEX)
        batch_op.drop_constraint(TAG_LINK_RESOURCE_CHECK_CONSTRAINT, type_="check")
        batch_op.drop_constraint(
            TAG_LINK_EXPERIMENT_RUN_ID_TAG_ID_UNIQUE_CONSTRAINT, type_="unique"
        )
        batch_op.drop_constraint(
            TAG_LINK_EXPERIMENT_ID_TAG_ID_UNIQUE_CONSTRAINT, type_="unique"
        )
        batch_op.drop_constraint(
            TAG_LINK_AGENT_VERSION_ID_TAG_ID_UNIQUE_CONSTRAINT, type_="unique"
        )
        batch_op.drop_constraint(
            TAG_LINK_COHORT_VERSION_ID_TAG_ID_UNIQUE_CONSTRAINT, type_="unique"
        )
        batch_op.drop_constraint(
            TAG_LINK_COHORT_ID_TAG_ID_UNIQUE_CONSTRAINT, type_="unique"
        )
        batch_op.drop_constraint(
            TAG_LINK_SESSION_ID_TAG_ID_UNIQUE_CONSTRAINT, type_="unique"
        )
        batch_op.drop_constraint(
            TAG_LINK_EXPERIMENT_RUN_ID_FOREIGN_KEY, type_="foreignkey"
        )
        batch_op.drop_constraint(TAG_LINK_EXPERIMENT_ID_FOREIGN_KEY, type_="foreignkey")
        batch_op.drop_constraint(
            TAG_LINK_AGENT_VERSION_ID_FOREIGN_KEY, type_="foreignkey"
        )
        batch_op.drop_constraint(
            TAG_LINK_COHORT_VERSION_ID_FOREIGN_KEY, type_="foreignkey"
        )
        batch_op.drop_constraint(TAG_LINK_COHORT_ID_FOREIGN_KEY, type_="foreignkey")
        batch_op.drop_constraint(TAG_LINK_SESSION_ID_FOREIGN_KEY, type_="foreignkey")
        batch_op.drop_column("experiment_run_id")
        batch_op.drop_column("experiment_id")
        batch_op.drop_column("agent_version_id")
        batch_op.drop_column("cohort_version_id")
        batch_op.drop_column("cohort_id")
        batch_op.drop_column("session_id")

    op.execute("DELETE FROM tag_link")
    with op.batch_alter_table("tag_link", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("resource_type", sa.String(length=32), nullable=False)
        )
        batch_op.add_column(sa.Column("resource_id", sa.Uuid(), nullable=False))
        batch_op.create_index(
            OLD_TAG_LINK_RESOURCE_INDEX,
