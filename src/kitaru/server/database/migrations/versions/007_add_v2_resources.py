...
Revision ID: 007_add_v2_resources
Revises: 006_add_device
Create Date: 2026-07-29 21:06:30.944266
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("run_working_dir", sa.String(), nullable=True),
        sa.Column(
            "run_env",
            postgresql.JSONB(none_as_null=True, astext_type=sa.Text()),
            nullable=True,
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["owner_id"], ["account.id"], name="fk_cohort_owner_id"
        ),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_table(
        ...
        sa.Column("numerical_value", sa.Float(), nullable=True),
        sa.Column("string_value", sa.String(), nullable=True),
        sa.Column("explanation", sa.String(), nullable=True),
        sa.CheckConstraint(
            "(data_type IN ('float', 'bool') AND numerical_value IS NOT NULL "
            "AND string_value IS NULL) OR (data_type = 'str' "
            "AND numerical_value IS NULL AND string_value IS NOT NULL) "
    ...
    with op.batch_alter_table("evaluation", schema=None) as batch_op:
        ...
        batch_op.create_index(
            "ix_evaluation_session_id_name_manual",
            ["session_id", "name"],
            unique=True,
...
def downgrade() -> None:
    ...
    op.drop_constraint("fk_session_task_id", "session", type_="foreignkey")
    with op.batch_alter_table("task", schema=None) as batch_op:
        batch_op.drop_index(
            "ix_task_staleness",
            postgresql_where=sa.text("status IN ('claimed', 'running')"),
        ...
        batch_op.drop_index(
            "ix_task_pending_id", postgresql_where=sa.text("status = 'pending'")
        )
