...
class InvalidDisplayVersion(ValidationError):
    """Raised when a display version exceeds the length limit."""


def validate_display_version(value: str) -> str:
    """Validate a display version against the length limit.

    Args:
        value: Display version to validate.

    Raises:
        InvalidDisplayVersion: ``value`` exceeds the length limit.

    Returns:
        Validated display version.
    """
    if len(value) > MAX_DISPLAY_VERSION_LENGTH:
        raise InvalidDisplayVersion(
            f"Display version exceeds {MAX_DISPLAY_VERSION_LENGTH} characters"
        )
    return value
...
class AgentVersion(DomainModel):
    ...
    agent_id: uuid.UUID
    version: int = 0
    display_version: DisplayVersion | None = None
