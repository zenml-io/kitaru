...
class Cohort(DomainModel):
    ...
    def check_members(self, session_ids: Sequence[uuid.UUID]) -> None:
        """Validate a member session list is non-empty with no duplicates.

        Existence of each session and its agent match are validated by the
        caller, which has access to the session repository.

        Raises:
            ValidationError: The list is empty or a session id repeats.
        """
        if not session_ids:
            raise ValidationError(f"Cohort {self.id} must have at least one session")
