...
class ReplayConfig(DomainModel):
    ...
    def check_standalone(self) -> None:
        """Reject a cohort-scoped history tool config.

        A standalone replay has no cohort to draw history from, unlike an
        experiment, which allows cohort scope.

        Raises:
            ValidationError: A tool config uses cohort-scoped history.
        """
        configs = [self.tool_policy.default, *self.tool_policy.tools.values()]
        for config in configs:
            if (
                isinstance(config, HistoryConfig)
                and config.scope is HistoryScope.COHORT
            ):
