...
def _readme(required_environment_names: tuple[str, ...]) -> str:
    forwarded = " ".join(
        f"--env.agent.harness.forward-env {name}" for name in required_environment_names
    )
    suffix = f" {forwarded}" if forwarded else ""
...
```bash
uv sync
uv run eval kitaru-verifiers-v1 --model MODEL --env.agent.runtime.type docker{suffix}
```

Verifiers supplies the Runtime sandbox and intercepts the model calls that form the
v1 trace. The generated harness copies `agent_source/` into the runtime, sets
`KITARU_TASK_INPUTS`, model endpoint variables, and MCP URLs, then runs the
registered shell command unchanged.
