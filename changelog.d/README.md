...
Name the file `<pr-number>.<section>.md`, where `<section>` is one of `added`, `changed`, `deprecated`, `removed`, `fixed`, or `security`. When the pull request does not exist yet, any slug such as `agent-delete.added.md` works in place of the number. The file holds one or more Markdown list items, each on a single line, exactly as they should appear under that section:
