...
So I built myself a small agent. It reads across a few sources, scores what it finds, and prints a short brief I can read in two minutes. I put it on Kitaru and PydanticAI mostly because we use Kitaru ourselves and I wanted a non-trivial example to break, replay, and poke at on a real workload. The full thing lives at [examples/news_scout](https://github.com/zenml-io/kitaru/tree/develop/examples/news_scout).
...
## Baking keys into the image

The flow declares its own image, and `_collect_env()` reads the keys I have locally and bakes them into the container when Kitaru builds it for a remote stack:
...
Baking keys into images is fine for a personal scout running on my own infra. It is *not* fine for shared infrastructure. Keys in images leak the moment the image leaks. For anything beyond a one-developer toy, use `kitaru secrets` and model aliases. I went laptop-grade on purpose here because I'm the only consumer and I'd rather optimize for "works after a `kitaru stack use`" than build a credential pipeline for myself.
...
# Drop your keys in examples/news_scout/.env, then:
