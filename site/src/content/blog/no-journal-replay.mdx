...
image: "https://assets.kitaru.ai/content/blog/8a6acec1/no-journal-replay.avif"
---
...
This creates one universal constraint: **your code must produce the exact same sequence of operations on replay as it did originally.** Change code while executions are in-flight and the journal no longer matches. Everything breaks.

Kitaru sidesteps this entirely. And for AI agents, that changes everything.

## The determinism tax

Here's what journal replay means in practice. Inside your workflow code, you cannot:
...
Every side effect must go through a special API — `ctx.run()` in Restate, Activities in Temporal, `step.run()` in Inngest. Miss one, and you get a non-determinism error that's hard to debug and impossible to Google.

For a payment processing workflow, this is manageable. The logic is predictable. You know the shape of execution upfront.

For an AI agent? It's a nightmare. Agents are inherently non-deterministic. They call LLMs that return different responses each time. They make tool-use decisions at runtime. They branch unpredictably. Forcing this into a deterministic journal model means fighting your execution engine at every step.
...
No journal. No replay. No determinism constraint.
...
AI agents are a different beast. They're long-running, expensive, unpredictable, and you want humans in the loop. The ML pipeline world — where each step is a pod that caches its output — turns out to be a much better foundation for that workload than replaying journals.

Kitaru connects these two worlds. ZenML's "spin up a step, cache the output, skip on rerun" machinery, repurposed as a durable execution model for agents. No journal. No determinism. Just cached checkpoints and normal Python.

## Continue in the docs

- [Checkpoints](/docs/concepts/checkpoints)
- [Replay and Overrides](/docs/getting-started/replay-and-overrides)
- [Wait and Resume](/docs/getting-started/wait-and-resume)
