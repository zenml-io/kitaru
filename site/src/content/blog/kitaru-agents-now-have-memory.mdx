...
category: "Kitaru"
image: "https://assets.kitaru.ai/content/blog/502b228d/kitaru-memory.avif"
...
{/* Screenshot placeholder: dashboard view showing a run/checkpoints alongside memory available at each stage. Good place for the first "photo-worthy" UI screenshot. */}
...
{/* Diagram placeholder: "boxes vs shelves" diagram. Left side: checkpoint artifacts as boxes attached to specific runs. Right side: memory as stable shelves keyed by scope_type + scope + key, with versions stacked behind each shelf label. */}
...
Distilled from `examples/memory/flow_with_memory.py`, it looks like this:
...
Memory reads and writes happen in the flow body, not inside checkpoints. That is deliberate: checkpoints can be cached or replay-skipped, while memory I/O needs to actually run. The [memory concept docs](/docs/concepts/memory#where-memory-calls-are-allowed) cover the rule in more detail.
...
{/* Diagram placeholder: version timeline for one key. Show v1 seeded, v2 updated by execution, v3 tombstone; latest get() points to None after tombstone while history still shows all versions. */}
...
{/* Screenshot placeholder: MCP/tooling view where an assistant lists or reads memory entries. This is the second likely "photo-worthy" UI screenshot if available. */}
...
Memory is a foundation, not a claim that every memory-shaped feature is done.
...
```bash
uv run examples/memory/flow_with_memory.py --output json
...
- [Memory concept](/docs/concepts/memory)
- [Python memory reference](/docs/reference/python/memory)
- [CLI memory reference](/docs/cli/memory)
- [MCP server guide](/docs/agent-native/mcp-server)
