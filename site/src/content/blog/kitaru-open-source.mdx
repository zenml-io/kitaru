...
**Kitaru is not a tracing tool. Kitaru is not an agent framework. Kitaru is the execution and deployment layer that sits underneath both.**

Traces are passive — they tell you what happened after the fact, from a distance, in a dashboard that has nothing to do with your code. They don't help your agent recover from where it failed. They don't let you pause mid-run and get a human to approve before the expensive step. They don't scale your execution out of your laptop and onto a real cloud environment. Most teams already have traces. What they need on top is execution that survives failures, control at the function level, human approval gates that actually pause the process, and the ability to run the same agent locally and in production without a rewrite.

Agent frameworks give building blocks to design AI agents — and you should use one if it fits your use-case. But most frameworks stop at the boundary of deployment and management. For example, the awesome [Pydantic AI](https://ai.pydantic.dev/) framework gives an abstraction where tools like Kitaru can be [plugged in to orchestrate and interface with the runtime](https://ai.pydantic.dev/durable_execution/prefect/#durable-agent). The OpenAI Agents SDK and CrewAI SDK have similar abstractions.
