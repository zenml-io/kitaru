...
I spent a lot of the last 3 years talking to teams building agents. Not demos — actual production systems. These deployments created actually business value.
...
I believe that having an army of agents running autonomously in the background and coordinating complex value-creation tasks will be fast be a norm worldwide. 
...
However, there is a natural incentive for most enterprises to invest in building infrastructure that allows these agents to break free of their hosts, and join a managed army working together autonomously and without human intervention (for the most part).

Digging deeper, this is fundamentally a platform problem. While the model providers are naturally inclined to eat that part of the stack, if past history is any indicator, a big portion of the market would do well to invest in building internal platform to deploy and run these agents themselves - on their terms. 

It is now clear that companies that will success with agents are the ones who have in-house knowledge on how to develop, deploy, and manage ambient agents at scale.
...
And run the same code above, and now the code runs as K8s jobs that are orchestrated centrally via the Kitaru server. You have full control over this orchestration setups, and it works on all major cloud providers. This mechanism is also extensible.

## What we're still building

The core is solid. A few things we're actively working toward:

**Multi-agent coordination** — agents calling subagents, tracking lineage across them, sharing context mid-run — is on the roadmap. The primitives exist, the polished end-to-end experience doesn't yet.
...
If your agents are dying in production and you're tired of restarting from scratch, give it a try. Or simple give us a star on [GitHub](https://github.com/zenml-io/kitaru) to show support!