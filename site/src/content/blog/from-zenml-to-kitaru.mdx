...
description: "We spent four years building ML pipeline infrastructure. Then agents showed up and we realized the next problem needed a new tool — not an extension of the old one."
date: 2026-03-10
author: "Hamza Tahir"
category: "Philosophy"
ogImage: "https://assets.kitaru.ai/content/blog/e3c1647c/from-zenml-to-kitaru.avif"
image: "https://assets.kitaru.ai/content/blog/e3c1647c/from-zenml-to-kitaru.avif"
---
...
They were hacking ZenML's pipeline model to fit a workload it wasn't built for. Dynamic DAGs. Conditional steps. State passed through artifact store workarounds. It worked, technically. But it was ugly, and the abstractions fought them at every turn.

The core issue: ZenML's mental model was "define a graph, then execute it." Agents don't have a graph. They have a loop with branching decisions that depend on LLM responses nobody can predict.

We could have bolted agent support onto ZenML. Added a "dynamic mode." Extended the step abstraction. Made the DAG optional. Some of our team argued for exactly that.
...
**Artifact versioning.** Every intermediate result stored, versioned, and comparable. This is just as valuable for agent checkpoints as it is for training pipeline outputs. We kept this.
...
**DAGs → normal Python.** The biggest change. ZenML pipelines were directed acyclic graphs — you defined steps and wired them together. Kitaru flows are just functions. `if/else`, `while`, `try/except` — they all work. The framework doesn't need to know the execution shape upfront.

**Steps → checkpoints.** A ZenML step was a unit of work in a graph. A Kitaru checkpoint is a function whose output you want to cache for crash recovery and replay. Subtle difference, but it changes how you think about decomposition. You don't checkpoint because it's a "step in the pipeline." You checkpoint because re-running that function would cost $2 in tokens.

**No first-class human interaction → `wait()`.** ZenML pipelines ran start to finish. There was no concept of pausing mid-execution for human input. For agents, this is essential. We built `wait()` as a core primitive, not an afterthought.
...
We considered it seriously. The argument for extending ZenML:

- Existing user base. Thousands of teams already using it.
- Shared infrastructure. Artifact store, metadata store, stack abstraction — all reusable.
- Less work. Extending is cheaper than building new.
...
- **Different audience.** ZenML users are ML engineers building training pipelines. Kitaru users are agent engineers building autonomous systems. Different people, different problems, different language.
...
## What we learned from four years of ZenML

The biggest lesson: **developer experience is the product.** Not the feature set. Not the architecture diagram. The experience of going from `pip install` to working code.
...
Kitaru is built on ZenML's engine. The artifact store, the metadata store, the orchestration layer — that's four years of battle-tested infrastructure. But the interface is completely new, designed for a different problem and a different user.

We're still early. The SDK is functional but not complete. The adapter ecosystem is growing. The dashboard is being built.

What we're confident about: agents need infrastructure that understands their specific needs — crash recovery without re-burning tokens, human oversight as a primitive, replay from any point, cost visibility per step. That's what we're building.
