...
image: "https://assets.kitaru.ai/content/blog/52f6129f/anatomy-of-a-production-agent.avif"
---
...
**Failure mode:** The LLM misunderstands the issue. Without a checkpoint, you'd never know — the agent would barrel ahead with a wrong plan.

### 2. Codebase exploration

The agent navigates the repo — finding relevant files, understanding architecture, tracing dependencies.
...
The agent creates a concrete implementation plan — which files to change, in what order, with what approach.
...
**Why this is a checkpoint:** This is usually the most expensive step — multiple LLM calls, each generating code. Caching this is critical for cost control.
...
That's normal Python. No graph. No YAML. No special control flow. But every meaningful step is checkpointed, there's a human approval gate, and the whole thing survives crashes.
...
**Real suspension.** The human approval step needs actual process suspension — not a polling loop burning compute while someone reviews a plan over lunch.
...
- [Tracked LLM Calls](/docs/getting-started/llm-calls)
