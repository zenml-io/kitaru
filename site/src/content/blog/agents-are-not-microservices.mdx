...
image: "https://assets.kitaru.ai/content/blog/deb278bd/agents-are-not-microservices.avif"
---

Temporal was born at Uber. It was designed for ride matching, payment processing, and order fulfillment — millions of short-lived, predictable workflows per second, each needing sub-100ms latency.
...
**Unpredictable execution paths.** The agent decides at runtime which tools to call, how many times to iterate, and when to ask a human. You can't predict the sequence of operations upfront. Forcing this into a deterministic model means fighting the framework.
