...
They don&rsquo;t compete. They compose. Kitaru ships a [first-class Pydantic AI adapter](/docs/concepts/flows) that tracks model requests and tool calls as child events under the enclosing checkpoint.
...
- **Execution placement.** `@checkpoint(runtime="isolated")` runs a specific step in its own pod or job on Kubernetes / Vertex / SageMaker / AzureML. Heavy or risky steps stay isolated; orchestration stays inline.
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: "First-class Kubernetes, Vertex, SageMaker, AzureML deployment", kitaru: true, competitor: false },
...
def review_flow(case: dict) -> str:
    ...
    # Compute released while we wait; survives crashes.
    ok = wait(name="approve", question=draft, schema=bool)
...
- Read the [Pydantic AI adapter docs](/docs/concepts/flows) for the composition pattern.
