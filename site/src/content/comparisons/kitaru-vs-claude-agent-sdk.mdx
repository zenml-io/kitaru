...
Kitaru is opinionated about that layer. The same flow code runs locally and then as a durable workflow on Kubernetes, AWS, GCP, or Azure. `kitaru build`, `kitaru serve`, and `kitaru deploy` turn your agent into a Docker image with durability baked in, and a configured stack lets platform teams standardize deployments for every team downstream.
</FeatureWithGraphic>
