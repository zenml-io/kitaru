...
- **Memory docs accuracy:** document the shipped memory surfaces precisely. Module-level `kitaru.memory.*` uses the currently configured scope and does not take per-call `scope=` arguments; `KitaruClient.memories`, CLI memory commands, and MCP memory tools use explicit scopes. Memory remains forbidden inside `@checkpoint`.
