...
- **Stack docs accuracy:** current shipped stack-create types on CLI/MCP are `local`, `kubernetes`, and `vertex`. The public Python SDK `kitaru.create_stack(...)` remains local-only, and SageMaker/AzureML should still be documented as unshipped.
