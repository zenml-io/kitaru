...
describe("generated conversation scorers", () => {
  ...
  it.each([0, 1])(
    "changing earlier message %s changes both scores",
    async (index) => {
  ...
  it.each(["running", "runtime_failure", "call_timeout"])(
    "rejects incomplete stop reason %s before judging",
    async (stop_reason) => {
      const input = view();
  ...
  it.each(["time_limit", "usage_limit", "turn_limit"])(
    "accepts a completed paired transcript stopped by %s but rejects a pending user",
    (stop_reason) => {
      const input = view();
      Object.assign(input.session.outputs as object, { stop_reason });
