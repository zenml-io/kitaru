...
Mastra 1.67 continues the model call in the background when this reader is left unconsumed or canceled, so Kitaru records the eventual completion. An observable Mastra abort is recorded as failed. This standalone command records a baseline and does not create a replay job; the adapter can replay a stream when a worker supplies `KITARU_REPLAY_ID`. User `prepareStep` and input processors, approval and resume modes, background execution, `untilIdle`, and secondary structured-output models remain unsupported; schema-only structured output is supported. Default-option and tool resolvers must be deterministic and side-effect-free because Mastra and Kitaru may call them more than once, with no exact invocation-count guarantee.
