...
This README is the runnable companion: setup, commands, and what you should see. The full stage-by-stage walkthrough lives in the docs tour at **[docs.zenml.io/user-guides/agents-guide](https://docs.zenml.io/user-guides/agents-guide/)**, and the [production notes](https://docs.zenml.io/user-guides/agents-guide/production-notes/) cover which pieces are teaching stand-ins and what to harden first.
...
|---|---|---|---|
| **[1. Durable agent](https://docs.zenml.io/user-guides/agents-guide/01-durable-agent/)**<br>`stage_1_basic_agent.py` | Key only | A PydanticAI agent inside a Kitaru flow; completed turns survive a crash | Re-run after a forced failure: checkpoint `default` returns `cached` (instant, $0) while `default_2` runs fresh |
| **[2. Sandbox](https://docs.zenml.io/user-guides/agents-guide/02-sandbox/)**<br>`stage_2_sandboxed_exec.py` | `bash setup.sh` | Shell commands run in a Docker container, not on your host | `[sandbox]` log lines; `cd /tmp` in turn 1 persists into turn 2; the agent reports Debian/root, not your machine |
| **[3. Skills](https://docs.zenml.io/user-guides/agents-guide/03-skills/)**<br>`stage_3_skills.py` | `bash setup.sh` | The agent reads its procedure from a `SKILL.md` file, not the system prompt | The agent calls `skill(list)` then `skill(read)` before acting; edit the markdown and re-run to change behavior with no Python edits |
| **[4. Credential proxy](https://docs.zenml.io/user-guides/agents-guide/04-credential-proxy/)**<br>`stage_4_credential_proxy.py` | `bash setup.sh` | A separate proxy holds the secret and adds the auth header; the worker never sees the token | `[proxy] injected headers for wiki.local: ['Authorization']` and a `200` from the mock; no `WIKI_TOKEN` exists inside the worker |
| **[5. Typed services](https://docs.zenml.io/user-guides/agents-guide/05-typed-services/)**<br>`stage_5_typed_services.py` | `bash setup.sh` | Structured calls go through a host-side `exec_service` dispatcher instead of shell `curl` | `lookup_wiki` and `publish_summary` produce no `[sandbox]` lines (they run host-side); the proxy stays idle for them |
| **[6. Human approval](https://docs.zenml.io/user-guides/agents-guide/06-hitl/)**<br>`stage_6_hitl.py` | `bash setup.sh` | `ask_question` pauses the flow with `kitaru.wait()` until a human answers | `Waiting on ask_question...`, then a durable pause; the flow resumes once you supply input (see [Stage 6](#stage-6-answering-the-human-pause)) |
...
The [production notes](https://docs.zenml.io/user-guides/agents-guide/production-notes/) walk through each teaching stand-in (the Docker sandbox, local skill files, the self-signed proxy, the mock services) and where you would swap in something your platform team trusts.
