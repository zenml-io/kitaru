...
`AGENT_HARNESS_PLATFORM_CREDENTIALS` env var (JSON: `{host: {header: value}}`),
authentication of incoming proxy requests goes through the
`AGENT_HARNESS_PLATFORM_PROXY_TOKEN` (basic-auth-as-bearer pattern).
"""
...
_CREDENTIALS: dict[str, dict[str, str]] = json.loads(
    os.environ.get("AGENT_HARNESS_PLATFORM_CREDENTIALS", "{}")
)
_PROXY_TOKEN = os.environ.get("AGENT_HARNESS_PLATFORM_PROXY_TOKEN", "")
_PROXY_AUTH_CHALLENGE_HEADERS = {
    "Proxy-Authenticate": 'Basic realm="agent-harness-platform-proxy"'
}
    ...
    def request(self, flow: HTTPFlow) -> None:
        ...
        for pattern, headers in _CREDENTIALS.items():
            ...
            if host == normalized_pattern or host.endswith(f".{normalized_pattern}"):
                ...
                print(
                    f"[agent-harness-platform-proxy] injected headers for {host}: "
                    f"{sorted(headers)}",
                    flush=True,
                )
                return
        print(
            f"[agent-harness-platform-proxy] no match for host={host!r} "
