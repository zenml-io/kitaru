...
- **Wrap a fast-agent app:** `examples/integrations/fast_agent_agent/fast_agent_adapter.py` uses real fast-agent objects, OpenAI gpt-5-nano by default, and a local tool with checkpoint-level usage/cost metadata
...
| fast-agent adapter example | `uv sync --extra fast-agent --no-dev` + `Python >=3.13.5,<3.14` + `OPENAI_API_KEY` |
...
- [integrations/fast_agent_agent/README.md](integrations/fast_agent_agent/README.md) — fast-agent adapter example with real fast-agent objects, an in-memory LLM, and a local tool
...
| [fast-agent adapter](integrations/fast_agent_agent/fast_agent_adapter.py) | `uv run python examples/integrations/fast_agent_agent/fast_agent_adapter.py` | `uv sync --extra fast-agent --no-dev` + `Python >=3.13.5,<3.14` + `OPENAI_API_KEY` | Wrap a real fast-agent `AgentApp`; OpenAI gpt-5-nano and a local `uppercase` tool show call-level checkpoints with usage/cost metadata | [fast-agent Adapter](https://docs.zenml.io/kitaru/adapters/fast-agent/) | [tests/test_fast_agent_example.py](../tests/test_fast_agent_example.py) |
...
18. `uv run python examples/integrations/langgraph_agent/langgraph_adapter.py --strategy graph_call` *(local interrupt/resume with stable thread_id; no API key)*; then try `--strategy calls` after installing `langgraph-openai` and setting `OPENAI_API_KEY`
19. Optional advanced LangGraph sandbox step: `uv run python examples/integrations/langgraph_agent/langgraph_adapter.py --strategy sandbox` *(requires `uv sync --extra local --extra langgraph-openai`, `OPENAI_API_KEY`, and an active stack with exactly one sandbox component)*
20. `uv run python examples/integrations/langgraph_agent/langgraph_streaming.py` *(local graph-call live events; no API key)*
21. `uv run python examples/integrations/fast_agent_agent/fast_agent_adapter.py` *(real fast-agent objects, OpenAI gpt-5-nano; requires `OPENAI_API_KEY`)*
22. `cd examples/end_to_end/openai_research_bot && uv run python research_bot.py "Your query" --max-searches 2` *(OpenAI planner → submitted searches → writer report)*
23. `cd examples/end_to_end/coding_agent && uv run python agent.py "Your task"` *(full agent with tools + HITL)*
24. `cd examples/end_to_end/news_scout && uv run python scout.py` *(granular-checkpoint agent with 4 tools, dashboard-readable final_report artifact)*
25. `uv run python examples/end_to_end/compliance_review/stage_1_single_turn.py` *(Claude Agent SDK audit; walk through Stage 1, Stage 2, and conversational Stage 4 to see checkpointing, replay, and wait/resume in turn)*
26. `uv run python examples/features/mcp/mcp_query_tools.py`
