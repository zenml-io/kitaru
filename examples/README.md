...
| [Coding agent](coding_agent/agent.py) | `uv run python -m examples.coding_agent.agent "Your task"` | `uv sync --extra local` + model alias / provider credentials | Full agent loop with LiteLLM tool calling, `kitaru.wait()` HITL, custom materializers, and artifact persistence | [Tracked LLM Calls](https://kitaru.ai/docs/getting-started/llm-calls) | — |
...
4. `uv run examples/execution_management/client_execution_management.py`
5. `uv run examples/execution_management/wait_and_resume.py`
...
9. `uv run python -m examples.coding_agent.agent "Your task"` *(full agent with tools + HITL)*
