...
- **Build a sandboxed, credential-isolated, profile-gated agent harness platform (the chapter-by-chapter starter kit):** `examples/end_to_end/agent_harness_platform/` — see also the [docs tour](https://kitaru.ai/docs/agent-harness-platform/).
...
- [end_to_end/agent_harness_platform/README.md](end_to_end/agent_harness_platform/README.md) — chapter-by-chapter starter kit for an internal agent harness platform: durable PydanticAI agent → DockerSandbox → skills as markdown → credential proxy → typed services → HITL via `kitaru.wait()`. See the [docs tour](https://kitaru.ai/docs/agent-harness-platform/) for the polished surface.
