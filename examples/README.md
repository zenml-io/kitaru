...
- **Wrap a fast-agent app:** `examples/integrations/fast_agent_agent/fast_agent_adapter.py` uses real fast-agent objects, an in-memory LLM, and a local tool with no provider key
...
| fast-agent adapter example | `uv sync --extra fast-agent --no-dev` + `Python >=3.13.5,<3.14` |
...
| [fast-agent adapter](integrations/fast_agent_agent/fast_agent_adapter.py) | `uv run python examples/integrations/fast_agent_agent/fast_agent_adapter.py` | `uv sync --extra fast-agent --no-dev` + `Python >=3.13.5,<3.14` | Wrap a real fast-agent `AgentApp`; an in-memory LLM and local `uppercase` tool show call-level checkpoints without a provider key | [fast-agent Adapter](https://docs.zenml.io/kitaru/adapters/fast-agent/) | [tests/test_fast_agent_example.py](../tests/test_fast_agent_example.py) |
...
21. `uv run python examples/integrations/fast_agent_agent/fast_agent_adapter.py` *(real fast-agent objects, in-memory LLM, no API key)*
