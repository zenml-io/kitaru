...
| [OpenAI Agents sandbox tool](integrations/openai_agents_agent/openai_agents_sandbox_tool.py) | `uv run python examples/integrations/openai_agents_agent/openai_agents_sandbox_tool.py` | `uv sync --extra local --extra openai-agents` + `OPENAI_API_KEY` + active stack with one sandbox | Let an OpenAI agent call `kitaru_sandbox_command`, which runs a command through the active stack's sandbox and returns compact JSON | [OpenAI Agents sandbox tool](../docs/book/adapters/openai-agents.md#active-stack-sandbox-command-tool) | — |
