...
- **Make an agentu workflow durable and forkable:** `examples/integrations/agentu_agent/agentu_record_replay.py` — researcher fan-out → analyst → writer with plain `agentu.Agent` calls as checkpoints, an agentu middleware that saves every inner LLM prompt/response as Kitaru artifacts, and two replay forks of the writer step (counterfactual analysis, model swap)
