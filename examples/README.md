...
- **Wrap an OpenAI Agents SDK agent:** `examples/integrations/openai_agents_v2/agent.py`; use `--help` for a no-cost source-only import check
...
| Source-only OpenAI Agents v2 example | `uv sync --project plugins` |
...
- [integrations/openai_agents_v2/README.md](integrations/openai_agents_v2/README.md) — non-streaming source-only adapter example with a no-cost help check and an opt-in real OpenAI run
...
| [OpenAI Agents v2 adapter](integrations/openai_agents_v2/agent.py) | `uv run --project plugins python -m examples.integrations.openai_agents_v2.agent --help` | `uv sync --project plugins`; a real run also needs `OPENAI_API_KEY` and standalone Kitaru identity or a worker task | Source-only non-streaming adapter import, native `RunResult`, and Kitaru session/node recording | [OpenAI Agents Adapter](https://docs.zenml.io/kitaru/adapters/openai-agents/) | `plugins/tests/adapters/openai_agents/test_runner.py` |
...
13. `uv run --project plugins python -m examples.integrations.openai_agents_v2.agent --help` *(source-only import/help check; no model call)*
...
24. Configure `examples/v2/mcp/.mcp.json.example` and inspect one page with the read-only native MCP server
