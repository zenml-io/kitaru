...
### Added
- Jobs generalize replays into one unit of executable work with a `kind` discriminator, managed via `/v1/jobs`. Replay jobs keep the replay semantics, and the new session-run kind runs an agent version on fresh inputs: `POST /v1/session-runs` creates a session-run job from an agent or explicit agent version with optional inputs, name, and execution target, and the recorded result session is linked back to the job. The job list filters by kind, run, original session, standalone, worker, heartbeat-projected status, and execution target, and `GET /v1/jobs/{id}/spec` returns a kind-discriminated spec.
- Workers, registered pool daemons that poll for jobs, managed via `/v1/workers`. `POST /v1/workers` upserts by name, refreshing the claim scope, metadata, and last seen time on re-registration, and worker responses report a computed `live` flag controlled by the new `KITARU_SERVER_WORKER_LIVENESS_TIMEOUT_SECONDS` setting. Workers claim through the global `POST /v1/jobs/claim` endpoint, which atomically claims up to `max_jobs` pending jobs within a scope and bumps the worker's last seen time on every call. A scope narrows by agent versions, job kinds, one experiment run, or one job plus its fan-out children, combining as a conjunction. An unpinned scope claims only pool-target work, a scope pinned to a run or job claims regardless of the execution target. `POST /v1/workers/{id}/heartbeat` reports the jobs a worker has in flight and answers with the ones it should abandon.
- Execution targets prepare on-demand execution. Run specs carry an optional `image` and a `default_execution_target`, experiment runs and standalone or session-run jobs are stamped with a `pool` or `on_demand` target, and creating `on_demand` work is rejected with a conflict when the resolved run spec has no image. Only the `JobExecutor` interface ships for now, so `on_demand` jobs stay pending until an executor implementation exists.
- Client-side worker exposed as a Python API in `kitaru.worker`. Build a `WorkerConfig` with a claim scope, concurrency, and an optional lifetime timeout, then `await Worker(config).run()` to claim and execute jobs until the scope drains or the worker is stopped. Every knob also reads from the environment as `KITARU_WORKER_<FIELD>`, so a deployment configures a worker without code, and the API connection comes from `KITARU_API_URL` and `KITARU_API_KEY`. The worker registers itself by name, defaulting to host and pid, batches heartbeats for its in-flight jobs, reacts to server-side abandonment, enforces the run spec timeout, and reports failures with a captured output tail. Plugin code and import payloads are cached on disk by content hash.
- Job-side execution package `kitaru.job`, the code that runs inside a job process. `python -m kitaru.job score` runs a scoring function, either imported from a `module:attribute` source or loaded from a registered single-file scorer, over a `kitaru.job.scorer.SessionView` and returns a score in 0..1. `python -m kitaru.job import` runs a registered importer that yields `ParsedSession` and `ParseFailure` items from a payload. Both write their result to a file the worker ships with the completion, so a score job's result is its score value and an import job's result is its stats object. Agent code reads its job context via `kitaru.job.job_id()` and `kitaru.job.job_inputs()`. Scorers and importers may declare their dependencies as PEP 723 inline script metadata.
- Worker-facing job execution workflow. Every claim ships the resolved spec incl. decrypted secret environment, also readable via `GET /v1/jobs/{id}/spec`, and workers report status transitions via `PATCH /v1/jobs/{id}`. A process that succeeded is reported as completed for every kind, with the job result attached, and the server validates the completion per kind: replays and session runs require a completed result session, score jobs a value in 0..1, imports a stats object. Sessions created with a `job_id` are linked as the job's result session, at most one per job, and replay-created sessions are stored with origin `replay`. `POST /v1/jobs/{id}/tool-lookup` resolves history tool policies server-side within their configured scope. Completing a replay job fans out its score jobs within the same request, and the transition that makes a run's last job terminal finalizes the run with an aggregate summary. `POST /v1/experiment-runs/{id}/cancel` cancels pending and claimed jobs immediately and drains running ones through the heartbeat path. Jobs with lost heartbeats are requeued lazily on claim, controlled by the new `KITARU_SERVER_JOB_HEARTBEAT_TIMEOUT_SECONDS` and `KITARU_SERVER_JOB_MAX_ATTEMPTS` settings. Jobs are released back to pending via `POST /v1/jobs/{id}/release`, re-run after a failure via `POST /v1/jobs/{id}/retry`, and deleted once finished via `DELETE /v1/jobs/{id}`. Terminal experiment runs are deleted together with their jobs via `DELETE /v1/experiment-runs/{id}`. A run whose jobs failed or timed out finalizes as failed with a count error, and run lists filter by experiment and status.
- Experiments over cohorts, managed via `/v1/experiments`. An experiment carries an inline replay config (execution override, tool policy as a discriminated union of passthrough, history, static, and LLM policies, and a scoring policy) that the server normalizes into an immutable config row. Name and description stay editable, while cohort and config changes are rejected once a run exists, and deleting an experiment with runs, or a cohort referenced by an experiment, returns a conflict.
- Experiment runs started via `POST /v1/experiments/{id}/runs`, creating one pending replay and job per cohort session stamped with the experiment's config and the resolved agent version, which defaults to the latest runnable version of the cohort's agent. Runs carry a per-experiment number and are read via `/v1/experiment-runs` with computed job progress, and `/v1/experiment-runs/{id}/jobs` lists a run's jobs.
- Replays as a resource of their own, managed via `/v1/replays`. `POST /v1/replays` creates a replay of one session together with the job executing it, with the tool policy defaulting to a history policy scoped to the original session. The replay carries the verdict its score jobs settle: `passed`, the weighted `score`, the per-scorer `scores`, and an `error` when a scorer did not complete. `GET /v1/replays` filters by experiment run, replayed session, and outcome, and `GET /v1/replays/{id}/diff` computes the full node-aligned diff between the replayed and the result session. A session or agent version referenced by a job can no longer be deleted, and run spec and capability changes on a replayed agent version return a conflict.
- Sessions and session nodes, the recording and import core, managed via `/v1/sessions`. Adapters open recorded sessions in progress, stream node batches to `/v1/sessions/{id}/nodes` (idempotent upserts on client-generated node ids, parent-before-child ordering enforced), and finish via `PATCH /v1/sessions/{id}`, which computes cost, token, and call count rollups from the nodes. Importers create imported sessions already terminal with a provider and external id, duplicates return a conflict. The server computes stable node path keys and tool call cache keys at ingest, `POST /v1/sessions/{id}/scores` merges scores latest-wins, and the session list filters by agent, version, origin, status, provider, external id, name, tag, time ranges, score presence, and cost and token ranges. Node lists exclude payloads unless requested.
- Cohorts, immutable groups of finished sessions of one agent, managed via `/v1/cohorts`. A cohort is created from an agent id and explicit session ids. Members are listed in position order via `/v1/cohorts/{id}/sessions`, name and description stay editable, and deleting a session that is a member of a cohort returns a conflict.
...
- Update endpoints distinguish omitted fields from explicit nulls: an explicit null clears a nullable field and is rejected with a validation error on non-nullable ones.
