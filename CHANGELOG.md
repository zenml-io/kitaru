...
- Added post-import insight generation. A bounded deterministic profiler turns normalized sessions into evidence-backed chart candidates and investigation prompts, while an optional two-call model pass selects the strongest findings and edits only non-numeric copy. Cards use the agent-scoped insight input contract and carry versioned evidence and coverage metadata; their stored prompts preserve the displayed description, deterministic facts, optional `check_first` caveat, and whether the supplied session IDs cover the full affected population or a retained subset. Standalone generation also returns diagnostics. The async `analyze_post_import_sessions` entrypoint accepts normalized analyzer session views and derives the agent and import context needed by each persisted card. Install the `insights` extra to use the OpenAI provider and dedicated-project Langfuse observation.
...
- Every import now includes the built-in `kitaru/post-import-insights` analyzer, using deterministic generation without model credentials by default. Explicit configuration of that analyzer is preserved without adding a duplicate. The built-in scans every imported session incrementally, including in-progress sessions, while bounding retained evidence, chart categories, and model input. Exact high-cardinality counts can use temporary disk storage; one full session must still fit in memory. Custom analyzers retain their list-based contract. Hosted workers can execute built-in analyzer tasks, and imports without eligible sessions complete analysis without creating cards. Insight listings support an `import_id` filter through the originating task; deleting the import job deletes that task and removes this association while direct agent filtering still finds the insight. Downgrading from analyzer provenance revision `017` to `016` clears deleted analyzer-version IDs from retained insights. Server-provided plugins and their versions are read-only through the API.
