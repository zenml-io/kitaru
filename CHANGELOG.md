...
- The server can start an ephemeral worker in a Modal sandbox when no live worker covers a new job's tasks, configured through `KITARU_SERVER_EPHEMERAL_WORKER__*` variables and the new `modal` extra. Workers started with `KITARU_WORKER_ID` skip registration and adopt their stored scope.
