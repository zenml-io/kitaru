...
- Workers select the providers whose credentials their environment holds with `--selector kitaru/requires-credentials=<provider>[,<provider>]` on `kitaru worker start`, and a worker that sets no such selector registers with an empty one. Either way a worker skips import and analysis tasks that need any other provider's credentials, so a worker selecting nothing only claims tasks whose credentials arrive through a connection or that need none. Set `KITARU_SERVER_EPHEMERAL_WORKER__SELECTORS`, or `server.ephemeralWorker.selectors` in the Helm chart, to add selectors to every ephemeral worker's scope. A label selector's `values` may now be empty, matching only tasks that lack the key, and the compact `--selector KEY=` form sets one.
