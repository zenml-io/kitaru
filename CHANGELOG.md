...
### Added
- A client configuration file, `config.json` in the Kitaru config directory, storing the server URL. Client construction resolves the server URL from `KITARU_API_URL`, falling back to the stored server URL, and authenticates through the credential store when the environment carries no credential.
