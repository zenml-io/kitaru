...
- The `kitaru/openai-post-import-insights` analyzer keeps model-written card copy whenever a card passes validation and falls back to deterministic wording per card instead of discarding every card at once. The editor no longer writes page framing, and card copy may restate numbers taken from the card's own facts or chart, plain comparatives, and word quantities; invented numbers, causal claims, and unsupported outcome claims are still rejected. A model request that fails or times out now fails the analyzer task instead of silently returning deterministic cards.
