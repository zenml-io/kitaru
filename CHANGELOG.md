...
- Trigger-mode replays. An agent version's run spec is now a discriminated union of the existing command spec and a trigger spec pointing at a python function, as `module:attribute`. The function runs the agent in the user's own system and returns the external id of the resulting session, for which the task runner creates a placeholder session in the new `pending_import` status. A later import of that external id adopts the placeholder, evaluations run against the imported content, and the replay and experiment run settle as usual. A configurable import deadline fails the replay when no import arrives.
