...
- Workers, registered pool daemons that poll for jobs, managed via `/v1/workers`. `POST /v1/workers` upserts by name, refreshing the served agents, metadata, and last seen time on re-registration, and worker responses report a computed `live` flag controlled by the new `KITARU_SERVER_WORKER_LIVENESS_TIMEOUT_SECONDS` setting. Workers claim through the global `POST /v1/jobs/claim` endpoint, which atomically claims up to `max_jobs` pending jobs scoped by served agents or a single experiment run, yields only pool-target work when unscoped, and bumps the worker's last seen time on every call.
...
- Client-side job runner exposed as a Python API. Call `kitaru.Runner(api_url, api_key).run_experiment_run(run_id)` to claim and execute an experiment run's jobs with bounded concurrency, `run_job(job_id)` for a standalone replay or session run, or `run_worker(agent_ids, stop)` for pool worker mode, a long-lived claim loop serving the given agents that drains in-flight jobs before stopping. Every mode registers the runner as a worker, named after host and pid by default, and the lower-level `kitaru.JobRunner` executes one already-claimed job for custom claim loops. The runner starts the agent command with the Kitaru env contract injected (`KITARU_API_URL`, `KITARU_API_KEY`, `KITARU_JOB_ID`, and `KITARU_INPUTS` when the encoded inputs fit an env var), heartbeats while the process runs, reacts to server-side cancellation, enforces the run spec timeout, and reports failures with a captured output tail. Scoring runs in the runner: scorer functions are imported from `module:attribute` sources via `kitaru.load_scorer`, receive a `kitaru.SessionView`, and return a score in 0..1, with `kitaru.evaluate_scoring_policy` applying weights, `fail_below` protectors, and the pass threshold. Originals missing baseline scores are scored and posted when the run requests it. Agent code reads its job context via `kitaru.job_id()` and `kitaru.job_inputs()`.
- Runner-facing job execution workflow. Runners fetch the resolved spec incl. decrypted secret environment via `GET /v1/jobs/{id}/spec`, report status transitions via `PATCH /v1/jobs/{id}`, and keep alive via `POST /v1/jobs/{id}/heartbeat`, whose response reports the job status and signals when the worker should stop. Sessions created with a `job_id` are linked as the job's result session, and replay-created sessions are stored with origin `replay`. `POST /v1/jobs/{id}/tool-lookup` resolves history tool policies server-side within their configured scope, and `GET /v1/jobs/{id}/diff` computes the full node-aligned diff between a replay's original and result session. Completing a replay job stores a scalar diff summary, and the transition that makes a run's last job terminal finalizes the run with an aggregate summary. `POST /v1/experiment-runs/{id}/cancel` cancels pending and claimed jobs immediately and drains running ones through the heartbeat path. Jobs with lost heartbeats are requeued lazily on claim, controlled by the new `KITARU_SERVER_JOB_HEARTBEAT_TIMEOUT_SECONDS` and `KITARU_SERVER_JOB_MAX_ATTEMPTS` settings. Standalone jobs are claimed via `POST /v1/jobs/{id}/claim`, released back to pending via `POST /v1/jobs/{id}/release`, re-run after a failure via `POST /v1/jobs/{id}/retry`, and deleted once finished via `DELETE /v1/jobs/{id}`. Terminal experiment runs are deleted together with their jobs via `DELETE /v1/experiment-runs/{id}`. A run whose jobs failed or timed out finalizes as failed with a count error, and run lists filter by experiment and status.
...
- Experiment runs started via `POST /v1/experiments/{id}/runs`, creating one pending replay job per cohort session stamped with the experiment's config and the resolved agent version, which defaults to the latest runnable version of the cohort's agent. Runs carry a per-experiment number and are read via `/v1/experiment-runs` with computed job progress, and `/v1/experiment-runs/{id}/jobs` lists a run's jobs with their diff summaries.
- Standalone replays of single sessions via `POST /v1/replays`, returning a replay-kind job with the tool policy defaulting to a history policy scoped to the original session. A session or agent version referenced by a job can no longer be deleted, and run spec and capability changes on a replayed agent version return a conflict.
