...
### Added
- A source-only, non-streaming OpenAI Agents adapter that preserves the native `RunResult` while recording each run as a Kitaru session with root and observed activity nodes.
