...
- Added post-import insight generation. A bounded deterministic profiler turns normalized sessions into evidence-backed chart candidates and investigation prompts, while an optional two-call model pass selects the strongest findings and edits only non-numeric copy. Results use the agent-scoped insight input contract and carry versioned evidence, coverage, and diagnostics metadata. Install the `insights` extra to use the OpenAI provider and dedicated-project Langfuse observation.
