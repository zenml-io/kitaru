...
- Session node listings accept a `node_type` filter through the API, SDK, and MCP, or repeatable `--node-type` options in `kitaru session nodes`. Selected types match before pagination, so LLM and tool calls can be read without paging past framework spans. Omitting the filter still returns every node type.
