...
- Every import now includes the built-in `kitaru/post-import-insights` analyzer, using deterministic generation without model credentials by default. Explicit configuration of that analyzer is preserved without adding a duplicate. The built-in loads at most 250 full sessions, selected by UUID, and reports sampled coverage; custom analyzers still receive every eligible session. Hosted workers can execute built-in analyzer tasks, and imports without eligible sessions complete analysis without creating cards. Insight listings support an `import_id` filter through the originating task; deleting that task removes this association. Server-provided plugins and their versions are read-only through the API.
