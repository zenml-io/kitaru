...
### Added
- Worker pools, managed via `/v1/worker-pools` and `kitaru worker pool` commands. A pool is a named claim scope (task kinds and label selectors) that a worker joins with `KITARU_WORKER_POOL` or `kitaru worker start --pool` instead of declaring its own scope. Claims resolve the pool's scope at claim time, so editing a pool retargets its live workers without restarts.
- `GET /v1/worker-pools/{pool}/stats` (pool id or name) returns `pending_tasks`, `in_flight_tasks`, `oldest_pending_seconds`, `live_workers`, and `capacity` as the demand signal for autoscalers. Workers report their `concurrency` at registration and capacity sums it across the pool's live workers.
- Workers release unfinished tasks back to the queue instead of canceling them when shut down past the drain timeout, so autoscaler scale-in does not turn into task failure. Executors may write the `pending` status to hand a claimed task back.
- A `kitaru-worker` Helm chart under `helm-worker/` deploys a pool-joined worker Deployment with optional KEDA autoscaling on the pool stats endpoint, including scale to zero.
- A worker pool supervisor example under `examples/features/worker_pool_supervisor/` polls pool stats and scales local worker subprocesses, the bare-metal counterpart of the KEDA setup.
...
- Jobs and tasks, managed via `/v1/jobs` and `/v1/tasks`, running agent runs, evaluations, and imports through workers that claim tasks and send heartbeats. Every job carries a server-stamped `kind` naming the workflow that created it, usable in job list filters. The new `kitaru.worker` and `kitaru.task` packages run the worker process and give task code access to task-scoped accessors.
...
- Accounts carry a `metadata` dictionary, set through `PATCH /v1/accounts/{id}`. An account can only write its own metadata, and unlike active state and password it is writable under every auth scheme. A PATCH carrying `metadata` replaces the map whole.
- `PATCH /v1/accounts/{id}` rejects a password write for any account other than the caller's own. Changing your own password requires the current one in `old_password`. Active state is no longer writable through PATCH.
- Account activation tokens. An account created without a password starts inactive and its create response carries a one-time `activation_token`. `POST /v1/accounts/{id}/deactivate` deactivates an account and mints a fresh token the same way, and rejects the calling account. `POST /v1/accounts/{id}/activate` takes that token plus a new password, activates the account, and spends the token. The activate route is unauthenticated, because a pending account cannot log in until it holds a password. Both tokens arrive on an `AccountActivationTokenResponse`, so the plain `AccountResponse` never carries token material.
