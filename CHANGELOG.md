...
- Added the independently packaged `kitaru-post-import-insights` analyzer. A bounded deterministic profiler turns normalized sessions into evidence-backed chart candidates and investigation prompts, while an optional two-call model pass selects the strongest findings and edits only non-numeric copy. Cards carry versioned evidence and coverage metadata; their stored prompts preserve the displayed description, deterministic facts, optional `check_first` caveat, and whether the supplied session IDs cover the full affected population or a retained subset. The plugin fetches sessions through the SDK and includes its OpenAI and Langfuse dependencies. The server registers its exact package requirement without installing the implementation.
...
- Every import now includes the built-in `kitaru/post-import-insights` analyzer, using deterministic generation without model credentials by default. Explicit configuration of that analyzer is preserved without adding a duplicate. The plugin scans every imported session incrementally, including in-progress sessions, while bounding retained evidence, chart categories, and model input. Exact high-cardinality counts can use temporary disk storage; one full session must still fit in memory. Hosted workers can execute built-in analyzer tasks, and imports without sessions complete analysis without creating cards. Insight listings support an `import_id` filter through the originating task; deleting the import job deletes that task and removes this association while direct agent filtering still finds the insight. Downgrading from analyzer provenance revision `017` to `016` clears deleted analyzer-version IDs from retained insights. Server-provided plugins and their versions are read-only through the API.
