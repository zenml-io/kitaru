...
- The server can start an ephemeral worker in a Modal sandbox when no live worker covers a new import job's task, configured through `KITARU_SERVER_WORKER_LAUNCHER__*` variables and the new `modal` extra. Workers started with `KITARU_WORKER_ID` skip registration and adopt their stored scope.
