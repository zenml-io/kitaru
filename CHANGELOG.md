...
### Added
- Deploy-time trace importers and persisted background import jobs, exposed through `/v1/importers` and `/v1/import-jobs`. The built-in Langfuse JSONL importer auto-detects trace, observation, and ingestion-event exports, groups Langfuse sessions into multi-turn Kitaru sessions, maps unknown observations to spans, and records conservative replay readiness. Uploads target one explicit agent version, isolate per-session failures, discard raw files after processing, deduplicate exact normalized snapshots, and create immutable revisions for changed evidence.
...
- Runner-facing replay execution workflow. Runners claim pending replays via `POST /v1/experiment-runs/{id}/claim` (atomic, safe for parallel workers, moving the run to running on the first claim), fetch the resolved spec incl. decrypted secret environment via `GET /v1/replays/{id}/spec`, report status transitions via `PATCH /v1/replays/{id}`, and keep alive via `POST /v1/replays/{id}/heartbeat`, whose response signals cancellation. Sessions created with a `replay_id` are stored with origin `replay` and linked as the replay's result session. `POST /v1/replays/{id}/tool-lookup` resolves history tool policies server-side within their configured scope, and `GET /v1/replays/{id}/diff` computes the full node-aligned diff between the original and result session. Completing a replay stores a scalar diff summary, and the transition that makes a run's last replay terminal finalizes the run with an aggregate summary. `POST /v1/experiment-runs/{id}/cancel` cancels pending and claimed replays immediately and drains running ones through the heartbeat path. Replays with lost heartbeats are requeued lazily on claim, controlled by the new `KITARU_SERVER_REPLAY_HEARTBEAT_TIMEOUT_SECONDS` and `KITARU_SERVER_REPLAY_MAX_ATTEMPTS` settings.
...
- Cohorts, immutable groups of finished sessions of one agent, managed via `/v1/cohorts`. A cohort is created from explicit session ids or from the session list filter, which the server resolves in full and stores as a provenance snapshot. Members are listed in position order via `/v1/cohorts/{id}/sessions`, name and description stay editable, and deleting a session that is a member of a cohort returns a conflict.
