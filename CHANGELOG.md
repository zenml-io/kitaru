...
- Client-side replay runner exposed as a Python API. Call `kitaru.Runner(api_url, api_key).run_experiment_run(run_id)` to claim and execute an experiment run's replays with bounded concurrency, or `run_replay(replay_id)` for a standalone replay. The runner starts the agent command with the Kitaru env contract injected (`KITARU_API_URL`, `KITARU_API_KEY`, `KITARU_REPLAY_ID`, and `KITARU_INPUTS` when the encoded inputs fit an env var), heartbeats while the process runs, reacts to server-side cancellation, enforces the run spec timeout, and reports failures with a captured output tail. Scoring runs in the runner: scorer functions are imported from `module:attribute` sources via `kitaru.load_scorer`, receive a `kitaru.SessionView`, and return a score in 0..1, with `kitaru.evaluate_scoring_policy` applying weights, `fail_below` protectors, and the pass threshold. Originals missing baseline scores are scored and posted when the run requests it. Agent code reads its replay context via `kitaru.replay_id()` and `kitaru.replay_inputs()`.
- Runner-facing replay execution workflow. Runners claim pending replays via `POST /v1/experiment-runs/{id}/claim` (atomic, safe for parallel workers, moving the run to running on the first claim), fetch the resolved spec incl. decrypted secret environment via `GET /v1/replays/{id}/spec`, report status transitions via `PATCH /v1/replays/{id}`, and keep alive via `POST /v1/replays/{id}/heartbeat`, whose response reports the replay status and signals when the worker should stop. Sessions created with a `replay_id` are stored with origin `replay` and linked as the replay's result session. `POST /v1/replays/{id}/tool-lookup` resolves history tool policies server-side within their configured scope, and `GET /v1/replays/{id}/diff` computes the full node-aligned diff between the original and result session. Completing a replay stores a scalar diff summary, and the transition that makes a run's last replay terminal finalizes the run with an aggregate summary. `POST /v1/experiment-runs/{id}/cancel` cancels pending and claimed replays immediately and drains running ones through the heartbeat path. Replays with lost heartbeats are requeued lazily on claim, controlled by the new `KITARU_SERVER_REPLAY_HEARTBEAT_TIMEOUT_SECONDS` and `KITARU_SERVER_REPLAY_MAX_ATTEMPTS` settings. Standalone replays are claimed via `POST /v1/replays/{id}/claim`, released back to pending via `POST /v1/replays/{id}/release`, re-run after a failure via `POST /v1/replays/{id}/retry`, and deleted once finished via `DELETE /v1/replays/{id}`. Terminal experiment runs are deleted together with their replays via `DELETE /v1/experiment-runs/{id}`. A run whose replays failed or timed out finalizes as failed with a count error, run lists filter by experiment and status, and replay lists filter by run, worker, and heartbeat-projected status.
...
- Experiment runs started via `POST /v1/experiments/{id}/runs`, creating one pending replay per cohort session stamped with the experiment's config and the resolved agent version, which defaults to the latest runnable version of the cohort's agent. Runs carry a per-experiment number and are read via `/v1/experiment-runs` with computed replay progress, and `/v1/experiment-runs/{id}/replays` lists a run's replays with their diff summaries.
- Standalone replays of single sessions via `POST /v1/replays`, with the tool policy defaulting to a history policy scoped to the original session. The replay list filters by session, status, and standalone, and a session or agent version referenced by a replay can no longer be deleted. Run spec and capability changes on a replayed agent version return a conflict.
