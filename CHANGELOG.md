...
### Added
- A client configuration file, `config.json` in the Kitaru config directory, storing the active server URL. The default client resolves its server URL from `KITARU_API_URL`, falling back to the stored active server, and authenticates through the credential store when the environment carries no credential.
