...
- Deleting an agent now deletes its versions, cohorts, experiments, investigations, sessions, and tasks in batched transactions instead of rejecting the delete with HTTP 409. Deleting a plugin or experiment cascades its versions, runs, and replays the same way. Deleting an agent version, cohort, or secret can now return HTTP 409 when an experiment run or agent version still references it. Deleting a session can now return HTTP 409 when an investigation or replay references it, in addition to a cohort version, and is no longer blocked by a task. Tasks are deleted with the agent version, plugin version, session, or blob they take as input. Tag links are removed with the resource they point at. Deleting an agent also removes standalone replays of its sessions, and two concurrent deletes of the same agent both succeed. Creating a session, session node, experiment, experiment run, cohort, cohort version, investigation, replay, or annotation whose parent was deleted concurrently now returns HTTP 404 instead of HTTP 500.
