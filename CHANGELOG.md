...
- Added evaluators to imports. Pass `evaluators` on `POST /api/v1/imports`, or `--evaluator` to `kitaru session import`, and every listed evaluator scores each eligible completed or failed session once the import finishes. A failed evaluator marks the job failed while the import's `stats` still records the parse outcome. Added `GET /api/v1/imports` and `GET /api/v1/imports/{import_id}`, with `client.imports.list(...)` and `client.imports.get(...)`, the `kitaru import list` and `kitaru import get` commands, and the `import` kind of the MCP `kitaru_activity_read` tool, to read imports back with their `stats` and `error`. Sessions created by an import carry `import_id`.
- Added analyzers, a plugin kind that reads a set of sessions at once and writes insights about the set. Register one with `kitaru analyzer register` and version it with `kitaru analyzer version register`, the same way as an evaluator, minus any agent scoping. Pass `analyzers` on `POST /api/v1/imports`, or `--analyzer` and `--analyzer-params` to `kitaru session import`, and each listed analyzer runs once over the import's eligible completed or failed sessions when its minimum session count is met, in parallel with the import's evaluator tasks. A failed analyzer marks the job failed while the import's `stats` still records the parse outcome. Insights written this way record the analyzer version, the task, and the params that produced them.
...
- `kitaru-pydantic-ai` expands support through the PydanticAI 2.40 minor line, with a supported range of `>=2.14.1,<2.41`.
...
- Imports with no eligible sessions create no evaluator tasks. Selected analyzers are recorded as completed with a skipped result, without running the analyzer.
...
- Added `POST /api/v1/ui/sample-data`, which seeds a workspace with the sample returns agent: ten imported sessions with their nodes and evaluations, a tagged baseline, a frozen cohort version, a registered script evaluator, an open investigation with node-pinned highlights, and an experiment that is defined but never started. An optional `agent_name` in the request body overrides the sample agent's name. A second call returns HTTP 409 while the agent name is registered.
