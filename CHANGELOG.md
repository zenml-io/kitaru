...
- Scope filters that reach through a relation: cohorts and evaluations filter on `agent_id`, evaluations also on `cohort_id` and `experiment_run_id`, experiment runs on `cohort_id`, `agent_id`, and `agent_version_id`, and replays on `result_session_id`. The evaluation run filter scopes by producing task rather than by session, so a run's baseline and result evaluations land in the same page, and `result_session_id` supports `is_null` to select the replays that have not produced a result. Scope fields accept `eq` and `in`; negation is expressed by wrapping the condition in `not`.
