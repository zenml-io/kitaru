...
- The server can start an ephemeral worker in a Modal sandbox when no live worker covers a new import job's task, configured through `KITARU_SERVER_EPHEMERAL_WORKER__*` variables and the new `modal` extra. Workers started with `KITARU_WORKER_ID` skip registration and adopt their stored scope.
...
- Expanded the MCP setup docs with a venv/PATH caveat (the common failure mode where Claude Code inherits its ephemeral_workers's PATH rather than a later-activated venv) and added `claude mcp add` with all three scope flags as an alternative to hand-editing `.mcp.json`
