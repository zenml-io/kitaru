...
run_test "OpenAI Agents sandbox tool import/contract" \
    $UV_RUN python -c 'from examples.integrations.openai_agents_agent import openai_agents_sandbox_tool as ex; agent = ex._build_agent(); tool = agent.tools[0]; schema = tool.params_json_schema; assert tool.name == "kitaru_sandbox_command"; assert set(schema["properties"]) == {"command", "cwd"}; assert "env" not in schema["properties"]; assert getattr(tool, "_kitaru_cache_identity")["max_chars"] == 4000'
