...
# GitBook output. Committed (GitBook's Git Sync serves tracked files) under
# docs/book/cli/, with the sidebar fragment spliced into toc.md between markers.
GITBOOK_OUTPUT_DIR = REPO_ROOT / "docs" / "book" / "cli"
GITBOOK_TOC = REPO_ROOT / "docs" / "book" / "toc.md"
CLI_TOC_START = "<!-- CLI:START (generated by generate_cli_docs.py) -->"
CLI_TOC_END = "<!-- CLI:END -->"
...
# ---------------------------------------------------------------------------
# GitBook rendering — same model, plain Markdown for docs/book/
# ---------------------------------------------------------------------------


def _escape_gitbook(text: str) -> str:
    """Escape angle brackets so GitBook's CommonMark parser keeps them literal.

    GitBook renders plain Markdown (not MDX), so braces need no escaping —
    only bare angle brackets, which would otherwise be read as inline HTML.
    """
    return text.replace("<", "&lt;").replace(">", "&gt;")


def _gitbook_child_link(child: CommandDoc) -> str:
    """Relative link from a group page to a child command page."""
    return f"{child.slug}/README.md" if child.subcommands else f"{child.slug}.md"


def render_command_page_gitbook(cmd: CommandDoc, *, is_root: bool = False) -> str:
    """Render a single command's GitBook Markdown page."""
    title = "CLI Reference" if is_root else cmd.invocation
    desc = cmd.summary or f"Reference for the `{cmd.invocation}` command."
    safe_desc = desc.replace('"', '\\"')

    lines: list[str] = ["---", f'description: "{safe_desc}"']
    if is_root:
        lines.append("icon: terminal")
    lines += ["---", "", f"# {title}", ""]

    if cmd.body:
        lines += [_escape_gitbook(cmd.body), ""]

    lines += ["## Usage", "", "```bash", cmd.usage, "```", ""]
    ...
    if is_root:
        lines += [
            ...
            "## Output formats",
            "",
            "Most agent-facing commands support `--output json` (or `-o json`) in "
            "addition to the default text output.",
            "",
            "- **Text output** is designed for people reading the terminal directly.",
            "- **JSON output** is designed for agents and scripts that need a stable "
            "structure.",
            "- Single-item commands emit `{command, item}`.",
            "- List commands emit `{command, items, count}`.",
            "- `kitaru executions logs --follow --output json` is the special case: it "
            "emits one JSON event per line while following the stream.",
            "",
        ]
    ...
    if cmd.parameters:
        ...
        for p in cmd.parameters:
            ...
            req_str = "Yes" if p.required else "No"
            lines.append(
                f"| {p.names_display} | {type_str} | {req_str} | "
                f"{p.default or ''} | {_escape_gitbook(p.help)} |"
            )
        lines.append("")
    ...
    if cmd.subcommands:
        ...
        for sub in cmd.subcommands:
            desc_text = _escape_gitbook(sub.summary) if sub.summary else ""
            lines.append(
                f"| [`{sub.name}`]({_gitbook_child_link(sub)}) | {desc_text} |"
            )
        lines.append("")
...
def write_gitbook_tree(root: CommandDoc, output_dir: Path) -> list[str]:
    """Write the GitBook CLI tree (groups → README.md, leaves → <slug>.md)."""
    created: list[str] = []

    def _write(cmd: CommandDoc, parent_dir: Path, *, is_root: bool = False) -> None:
        if is_root or cmd.subcommands:
            dir_path = parent_dir if is_root else parent_dir / cmd.slug
            dir_path.mkdir(parents=True, exist_ok=True)
            page = dir_path / "README.md"
            page.write_text(render_command_page_gitbook(cmd, is_root=is_root))
            created.append(str(page.relative_to(output_dir)))
            for sub in cmd.subcommands:
                _write(sub, dir_path)
        else:
            parent_dir.mkdir(parents=True, exist_ok=True)
            page = parent_dir / f"{cmd.slug}.md"
            page.write_text(render_command_page_gitbook(cmd))
            created.append(str(page.relative_to(output_dir)))

    _write(root, output_dir, is_root=True)
    return created


def render_toc_fragment(root: CommandDoc) -> str:
    """Render the nested GitBook SUMMARY list for the CLI tree."""
    lines: list[str] = ["* [CLI Reference](cli/README.md)"]

    def _walk(cmd: CommandDoc, depth: int) -> None:
        indent = "  " * depth
        path = "cli/" + "/".join(cmd.docs_path)
        if cmd.subcommands:
            lines.append(f"{indent}* [{cmd.name}]({path}/README.md)")
            for sub in cmd.subcommands:
                _walk(sub, depth + 1)
        else:
            lines.append(f"{indent}* [{cmd.name}]({path}.md)")
...
def splice_toc(fragment: str) -> None:
    """Replace the CLI region in docs/book/toc.md with the generated fragment."""
    toc = GITBOOK_TOC.read_text()
    pattern = re.compile(
        re.escape(CLI_TOC_START) + r".*?" + re.escape(CLI_TOC_END),
        re.DOTALL,
    )
    replacement = f"{CLI_TOC_START}\n{fragment}\n{CLI_TOC_END}"
    if not pattern.search(toc):
        raise SystemExit(
            f"CLI toc markers not found in {GITBOOK_TOC.relative_to(REPO_ROOT)}"
        )
    GITBOOK_TOC.write_text(pattern.sub(replacement, toc))
...
def main() -> int:
    ...
    # GitBook output: committed tree under docs/book/cli/ + spliced toc.md nav.
    if GITBOOK_OUTPUT_DIR.exists():
        shutil.rmtree(GITBOOK_OUTPUT_DIR)
    tmp_gb = Path(tempfile.mkdtemp(prefix="kitaru-cli-gitbook-"))
    try:
        gb_files = write_gitbook_tree(tree, tmp_gb)
        shutil.copytree(tmp_gb, GITBOOK_OUTPUT_DIR)
    finally:
        shutil.rmtree(tmp_gb, ignore_errors=True)
    splice_toc(render_toc_fragment(tree))
    print(
        f"Generated {len(gb_files)} GitBook files in "
        f"{GITBOOK_OUTPUT_DIR.relative_to(REPO_ROOT)}/ and spliced toc.md"
    )
